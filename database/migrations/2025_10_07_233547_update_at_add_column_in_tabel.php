...
return new class extends Migration
{
    ...
    protected function addForeignKeyIfNotExists($table, $column, $referencedTable, $referencedColumn)
    {
        try {
            ...
            foreach ($foreignKeys as $key) {
                // stdClass -> use property access
                if (isset($key->column_name) && $key->column_name === $column) {
                    $exists = true;
                    break;
                }
            }
            ...
            if (!$exists) {
                Schema::table($table, function (Blueprint $tbl) use ($column, $referencedTable, $referencedColumn) {
                    $tbl->foreign($column)
    ...
    protected function addUniqueConstraintIfNotExists($table, $columns)
    {
        try {
            ...
            // Group indexes by name to compare full column sets
            $byIndex = [];
            foreach ($indexes as $idx) {
                $name = $idx->key_name;
                if (!isset($byIndex[$name])) {
                    $byIndex[$name] = [
                        'non_unique' => (int) $idx->non_unique, // 0 = unique
                        'columns' => [],
                    ];
                }
                $byIndex[$name]['columns'][] = $idx->column_name;
            }
    ...
    {
        // One row per column in each index
        return \DB::select("
            SELECT 
