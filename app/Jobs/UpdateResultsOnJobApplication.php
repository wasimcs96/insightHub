...
class UpdateResultsOnJobApplication implements ShouldQueue
{
     ...
     */
    public function __construct($user_id, $position_id)
    {
    ...
    public function handle()
    {
        ...
        if (!$user) {
            \Log::info("User not found: ID {$this->user_id}");
            return;
        }

        $userResults = UserResult::where('user_id', $this->user_id)->get();
        $user_type = $user->role_name ?? 'employee';

        \Log::info("Starting report update for User ID: {$this->user_id}");

        if ($user->is_personality_motivation_completed == 1) {
            // TP Match Rate / CCS Match Rate / SSMR
            $ccs_match_rate = $this->updateCcsMatchRate($this->user_id, $this->position_id, $user,$userResults, $descriptors, $user_type);
            $user->talent_pillar_match_rate = $ccs_match_rate;
        }

        if ($user->is_work_interest_completed == 1) {
            // JMR
            $jmr = $this->updateJmr($this->user_id, $this->position_id, $user,$userResults, $descriptors, $user_type);
        ...
        if (($user->is_personality_motivation_completed == 1) && ($user->is_work_interest_completed == 1) && ($user->is_cognitive_ability_completed == 1)) {
            // BFR
            $soft_skill_score = $this->updateBfr($this->user_id, $this->position_id, $user,$userResults, $descriptors, $user_type, $jmr, $ccs_match_rate);
            $user->soft_skill_score = $soft_skill_score;
            
            // OMR
            $overall_match_rate = $this->updateOmr($this->user_id, $this->position_id, $user,$userResults, $descriptors, $user_type, $soft_skill_score);
            $user->match_rate = $overall_match_rate;
        }

        $user->save();

        \Log::info("Report update completed for User ID: {$this->user_id}");
    }
    ...
    private function insertInDb($user_id, $assessment_type, $result_type, $dimension, $score, $z_score, $percentage,$level, $level_description, $user_type, $descriptors) {
