...
class CompanyTechnicalSkillController extends Controller
{
    ...
    public function fetchTechSkills(Request $request)
    {
        ...
        if (!empty($jobPositionIds)) {
            $jobPositionsWithDepts = TenantDB::table('jobs')
        ...
        $query = TenantDB::table('master_technical_skills')
  ...
  $letterQuery = clone $query;
        ...
        $allLetters = $letterQuery
            ->selectRaw('UPPER(LEFT(master_technical_skills.name, 1)) as first_letter')
            ->groupBy(DB::raw('UPPER(LEFT(master_technical_skills.name, 1))'))
            ->orderByRaw('UPPER(LEFT(master_technical_skills.name, 1)) ASC')
            ->pluck('first_letter')
            ->sort()
            ->values()
            ->toArray();
        ...
        if ($request->ajax()) {
           ...
           if (!empty($hierarchicalStructure)) {
            foreach ($hierarchicalStructure as $item) {
                $q = TenantDB::table('master_technical_skills')
            ->leftJoin('sectors as s', 'master_technical_skills.sector_id', '=', 's.id')
            ->leftJoin('technical_skill_categories as c', 'master_technical_skills.category_id', '=', 'c.id')
            ->select([
                'master_technical_skills.id',
