...
class AiJobService
{
    // public function createJob(array $validatedData, array $requestData)
    // {
    //     dd($requestData);
    //     try {
    //         $job = Job::create([
    ...
    //             'heads' => $validatedData['heads'],
    //             'level' => $validatedData['level'],
    //             'org_department' => $validatedData['org_department'],
    //             'code' => $validatedData['position_code'],
    //             'position_code' => Job::generateUniquePositionCode(),
    //             'saved_job' => 1,
    ...
    //         $this->handleJobSkills($job->id, $requestData['skills'] ?? []);
    ...
    public function createJob(array $requestData)
{
    ...
    //         $softSkill = LlmSoftSkillDescription::create([
    //             'job_id' => $jobId,
    //             'soft_skill_id' => null, // If there's a relation, fetch the correct ID
    ...
    private function handleJobSkills($jobId, $skills)
{
    ...
    // private function handleJobSkills($jobId, $skills)
    // {
    //     $jobSkills = array_map(function ($skill) use ($jobId) {
    //         return [
    //             'job_id' => $jobId,
    //             'title' => $skill['competency'] ?? '',
    //             'level' => $skill['level'],
...
//         for ($i = 1; $i <= 3; $i++) {
//             $levelKey = "level_$i";
//             $knowledgeKey = "{$levelKey}_knowledge";
//             $abilityKey = "{$levelKey}_ability";

//             if (isset($skill[$levelKey])) {
//                 $jobSkillLevel = JobSkillLevel::create([
//                     'job_skill_id' => $jobSkill->id,
//                     'level' => $i,
//                     'title' => $skill[$levelKey],
//                     'created_at' => Carbon::now(),
//                     'updated_at' => Carbon::now(),
//                 ]);

//                 // Insert knowledge items
//                 if (isset($skill[$knowledgeKey])) {
//                     $knowledgeData = array_map(function ($knowledge) use ($jobSkillLevel) {
//                         return [
//                             'job_skill_level_id' => $jobSkillLevel->id,
//                             'knowledge' => $knowledge,
//                             'created_at' => Carbon::now(),
//                             'updated_at' => Carbon::now(),
//                         ];
...
//                     JobSkillKnowledge::insert($knowledgeData);
//                 }

//                 // Insert ability items
//                 if (isset($skill[$abilityKey])) {
//                     $abilityData = array_map(function ($ability) use ($jobSkillLevel) {
//                         return [
//                             'job_skill_level_id' => $jobSkillLevel->id,
//                             'ability' => $ability,
//                             'created_at' => Carbon::now(),
...
//                     JobSkillAbility::insert($abilityData);
//                 }
//             }
//         }
//     }
    ...
    private function handleTechnicalSkills($jobId, $technicalSkills)
{
    ...
    //         $function = [
    //             'job_id' => $jobId,
    //             'description' => $functionData['title'] ?? '',
    //             'created_at' => Carbon::now(), 
    //             'updated_at' => Carbon::now(), 
    //         ];
    //         $functionId = JobCriticalFunction::create($function)->id;

    //         if (!empty($functionData['keys'])) {
    ...
    private function handleJobFunctions($jobId, $functions)
    {
        ...
        // Mapping key tasks to the inserted critical function IDs
        // foreach ($functions as $index => $functionData) {
        //     if (!empty($functionData['tasks'])) {
