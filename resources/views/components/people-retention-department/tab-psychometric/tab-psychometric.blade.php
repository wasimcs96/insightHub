...
<style>
  ...
  padding:80px 20px;
  text-align:center;
  min-height:340px;          /* gives that roomy feel */
  display:flex;
  align-items:center;
  justify-content:center;
}
...
{{-- {{ dd($bfrLevelCounts,$jmrLevelCounts,$flightRiskCounts,$workplaceAlignmentForecast,$catLevelCounts,$matchRateCounts,$growthPotentialCounts) }} --}}

@php
    // Safely sum each associative array (treat null as empty)
    $totals = [
        array_sum((array) $bfrLevelCounts),
        array_sum((array) $jmrLevelCounts),
        array_sum((array) $flightRiskCounts),
        array_sum((array) $workplaceAlignmentForecast),
        array_sum((array) $catLevelCounts),
        array_sum((array) $matchRateCounts),
        array_sum((array) $growthPotentialCounts),
        array_sum((array) $leadershipPotentialCounts),
    ];

    $hasAnyUsers = array_sum($totals) > 0;
@endphp
    ...
    <div class="psychometric-nav-content-box">
        <div class="psychometric-content">
            ...
            @if (!$hasAnyUsers)
                <div class="no-data card">
                    <p>Employees have not completed any assessments.<br>
                    Their results will be shown here once available.</p>
                </div>
            @endif
            {{-- Filter Buttons --}}
            @if ($hasAnyUsers)
            <div class="psychometric-filter-buttons">
                ...
                <div class="dropdown">
                    <button class="psychometric-filter-btn" type="button" id="psychometricPositionLevelChart" 
                        ...
                        <span style="color: #000">Position Level (Maximum 4 Levels)</span> 
                        <iconify-icon icon="tabler:chevron-down" width="16" height="16"></iconify-icon>
                    </button>
                    <div class="psychometric-dropdown-menu" style="width: 202px; max-height: 300px; overflow-y: auto; padding: 0px !important;">
                        <?php 
                        $levelNames = config('helpers.position_levels');
                        ...
                        ?>
                            <div class="d-flex gap-3 align-items-center psychometric-custom-check-input">
                                <input class="psychometric-checkbox-input" type="checkbox" 
                                       ...
                                       data-label="<?= $levelName ?>">
                                <label class="form-check-label d-flex align-items-center justify-content-between w-100 gap-3">
            ...
            <div class="aggregated-department-graph">
                <div class="aggregated-department-graph-container">
                    ...
                    <div class="technical-skill-match-rate chart-card">
                        <h2>Technical Skill Match Rate</h2>
                        <div id="technical-skill-match-rate-chart"></div>
                    </div>
        ...
        <div class="psychometric-content" id="tabPsychometric">
            <div class="psychometric-content-second-tab">
                <h2 class="psychometric-title">Individual Psychometric Report</h2>
            </div>
            <div class="filter-buttons d-flex align-items-center mb-14 justify-content-between" data-tab="individual">
                <div class="filter-container1">
                                    ...
                                    <div class="d-flex flex-column gap-1 custom-check-input">
                                        <div class="d-flex gap-3 align-items-center">
                                            <input class="interview-option" type="radio" 
                                                name="PositionLevelIndividualFilter"
                                                value="{{ $positionLevel['level'] }}" 
                                                data-target="PositionLevelIndividual" 
                                                data-label="{{ $label }}">
                                            <label class="form-check-label d-flex align-items-center justify-content-between w-100 gap-3">
                                                <p class="m-0">{{ $label }}</p>
                    ...
                    <div class="dropdown">
                        <button class="filter-btn filter-btn1" type="button" id="JobPositionIndividual" data-bs-toggle="dropdown"
                        ...
                        <div class="dropdown-menu mt-4" style="width: 210px; max-height: 300px; overflow-y: auto; padding: 0px !important;">
                           @foreach($levelsByPositionPsychometric as $jobPosition)
                            ...
                            <div class="d-flex justify-content-end gap-4 align-items-center button-div">
                                <button class="resetBtn border-0" data-target="JobPositionIndividual">Reset</button>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu mt-4" style="width: 202px; padding: 0px !important;">
                            ...
                            <div class="d-flex justify-content-end gap-4 align-items-center button-div">
                                <button class="resetBtn border-0" data-target="AssessmentCompletion">Reset</button>
                                <button class="filter border-0">Filter</button>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    window.bfrLevelData = @json(isset($talentInsightTotal) ? $talentInsightTotal : []);
    ...
    //     let selectedLevels = Array.from(
    //         document.querySelectorAll('.psychometric-checkbox-input:checked')
    //     ).map(cb => cb.value.replace('Level ', ''));

    //     // 👇 If no levels selected, use ALL available levels
    //     if (selectedLevels.length === 0) {
    //         selectedLevels = Array.from(
    //             document.querySelectorAll('.psychometric-checkbox-input')
    //         ).map(cb => cb.value.replace('Level ', ''));
    //     }
    ...
    // 👉 If no levels selected, don't send 'levels' at all
    const url = new URL(window.location.origin + `/admin/download-pdf/${departmentId}`);
    if (selectedLevels.length > 0) {
        url.searchParams.set('levels', selectedLevels.join(','));
    }
...
<script>
    ...
    button.addEventListener('click', function () {
        let container = this.closest('.employee-search');
        ...
        // Remove any applied active state if needed
        input.classList.remove("active-input");

        // Trigger any update functions you want
        updateAssessmentCompletionCounts();
        updateDependentDropdownsPsychometric();
    });
});
    ...
    $(document).ready(function() {
        const applyFilters = (page = 1) => {
            ...
            $.ajax({
                ...
                data: filters,
                success: (response) => {
    // if (response.tableHtml.trim() === '' || $(response.tableHtml).find('tr').length === 0) {
    //     // If no rows in the returned table HTML, show "No data available"
    //     $('#resultTable tbody').html(`
    //         <tr>
    //             <td colspan="6">
    ...
    //         </tr>
    //     `);
    //     $('.pagination-wrappe').html('');  // Clear pagination if no data
    // } else {
    ...
    updateUrlState(filters);  
        ...
        $('#tabPsychometric .employee-clearButton').click(function () {
            applyFilters(1);
        });
    ...
    $(document).on('click', '.psychometric-tab-nav', function () {
        // Reset radio button selection for Position Level filter
        $('input[name="PositionLevelIndividualFilter"]').prop('checked', false);

        // Reset button text back to default
        $('#PositionLevelIndividual span').text($('#PositionLevelIndividual').data('default-text'));

        // Trigger Reset button click to reset any other filter-related logic
        $('.resetBtn[data-target="PositionLevelIndividual"]').trigger('click');
    });
</script>
