<div id="offcanvasRight" class="offcanvas offcanvas-end" data-bs-scroll="false">
    ...
    <div class="offcanvas-body" style="padding-bottom: 120px;">
        ...
        <div class="filter-block mb-4" data-type="Job Positions" data-key="job_position">
            <div class="d-flex align-items-center justify-content-between">
        ...
        (function() {
            ...
            async function loadCompanies(buIds, term = '') {
                ...
                try {
                    ...
                    buildOptions(listEl, items, state.companyIds); // preserves checked (should be none on BU change)
                } catch (e) {
            ...
            function onApply(block) {
                ...
                if(key==='department'){
                    state.departmentIds = checked;
                    // Reset job positions
                    state.jobPositionIds = []; // NEW
                    if (state.departmentIds.length > 0) {
                      loadJobPositions(state.departmentIds); // NEW
                      setEnabled(document.querySelector('[data-key="job_position"]'), true); // NEW
                    } else {
                      buildOptions($('[data-list="job_position"]'), []); // NEW
                      setEnabled(document.querySelector('[data-key="job_position"]'), false); // NEW
                    }
                    updateSection(document.querySelector('[data-key="job_position"]')); // NEW
                  }
            ...
            function onReset(block) {
                ...
                if(key==='department') {
                    ...
                    setEnabled(document.querySelector('[data-key="job_position"]'), false);
                    updateSection(document.querySelector('[data-key="job_position"]'));
                  }
