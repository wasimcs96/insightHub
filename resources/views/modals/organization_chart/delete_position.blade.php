<div class="modal-body d-flex flex-column gap-3 p-4">
                        <div class="modal-content-p">
                            <p class="mb-2">Job Position Name:</p>
                            <p class="m-0"><b>{{ $title ?? '-'}}</b></p>
                        </div>

                        <div class="form-group">
                            <label>Reason for Job Position Removal</label>
                            <input type="text" class="form-control" id="reasonForDeletePosition" placeholder="Reason for Job Position Removal">
                        </div>
                    </div>
                    <div class="modal-footer justify-content-center border-0 pt-0">
                        <button type="button" class="cancel-button" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" class="orange-fill" data-modal-submit>Delete Position</button>
                    </div>