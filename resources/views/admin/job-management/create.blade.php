        ...
        <div id="kt_app_content" class="app-content  flex-column-fluid ">
            ...
            <div id="kt_app_content_container" class="app-container  container-xxl ">
                ...
                <div class="card  shadow-sm mb-4">
                    <div class="bg-primary card-header">
                        <h3 class="card-title text-white"><iconify-icon icon="prime:sparkles"
                                class="fs-2x mr-2"></iconify-icon>Role Based on Master JD</h3>
                    ...
                    <div class="card-body px-2 py-5">
                            ...
                            <div class="form-group d-flex justify-content-evenly col-lg-12 p-0">
                                <div class="col-lg-6">
                                    ...
                                    <input id="job_sector" class="form-select" placeholder="Select the Sector" readonly>
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <input id="job_role" name="job_role" class="form-select" placeholder="Select the Role" readonly>
                    ...
                    <div class="card-body padding-card">
                        ...
                        <div class="d-flex align-items-center row" id="riasec-block">
                            ...
                            <div class="fv-row fv-plugins-icon-container col-lg-3">
                                <button id="generate-riasec" type="button" class="btn btn-primary">Generate Riasec
                                    Using Job Role</button>
                            </div>
                        </div>
                        <div class="mb-5 row">
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body padding-card">
                        <div class="critical-functions-container" id="criticalFunctionsContainer">
                            {{-- JS will dynamically append items here --}}
                        </div>
                
                        <button type="button" id="add_new_function" class="add-function mt-5" style="background:#F7941C">
                            <iconify-icon icon="stash:plus-solid" style="font-size: 20px"></iconify-icon> Add New Function
                        </button>
                        <div id="validationMessage" style="color: red; display: none;">At least one Critical Work Function is required.</div>
                    </div>
                </div>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body padding-card">
                        <div class="skills-container row mb-3" id="technicalSkillsContainer">
                            {{-- JS will dynamically append items here --}}
                        </div>
                
                        <button type="button" id="add_new_skill" class="btn btn-primary">Add New Skill</button>
                        <div id="techvalidationmessage" style="color: red; display: none;">At least one Technical Skill is required.</div>
                    </div>
                </div>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body padding-card">
                        <div class="row mb-3" id="generalSkillsContainer">
                            {{-- JS will dynamically append generic skills here --}}
                        </div>
                
                        <button type="button" id="add_new_generic_skill" class="btn btn-primary">Add New Generic Skill</button>
                        <div id="genericSkillValidation" style="color: red; display: none;">At least one Generic Skill is required.</div>
                    </div>
                </div>
...
@section('scripts')
    ...
    {{-- <script>
        document.addEventListener('DOMContentLoaded', () => {
            const data = sessionStorage.getItem('selectedJD');
            if (!data) return;
            ...
            if (jd && jd.job_id) {
                document.getElementById('job_id').value = jd.job_id;
            }
            document.getElementById('job_role').value = jd.job_role || '';
            document.getElementById('job_role_desc').value = jd.description || '';
            document.getElementById('department').value = jd.department_id || '';
            document.getElementById('job_sector').value = jd.job_sector || '';
            document.getElementById('level-job').value = jd.level || '';
            document.getElementById('riasec').value = jd.top3riasec || '';
            document.getElementById('level').value = jd.level || '';
    
            // Populate Critical Work Functions
            // CRITICAL WORK FUNCTIONS
            const criticalContainer = document.getElementById('criticalFunctionsContainer');
            criticalContainer.innerHTML = ''; // Clear any existing fields
            (jd.criticalFunctions || []).forEach((item, index) => {
                const html = `
                    <div class="critical-function row mb-3">
                        <div class="col-lg-12">
                            <div class="d-flex gap-5 mb-3">
                                <button type="button" class="remove-function btn btn-outline btn-outline-primary d-flex justify-content-center" title="Remove function">
                                    <iconify-icon icon="gg:trash" class="fa-1-5"></iconify-icon>
                                </button>
                                <input type="hidden" name="functions[${index}][id]" value="${item.id || ''}">
                                <input type="text" class="form-control input-style" name="functions[${index}][title]" placeholder="Critical Work Function" value="${item.description || ''}" required>
                            </div>
                            <select class="form-control function-keys input-style select2 mb-3 col-lg-11" name="functions[${index}][keys][]" multiple="multiple">
                                ${(item.cwf_keys || []).map(k => `<option value="${k}" selected>${k}</option>`).join('')}
                            </select>
                        </div>
                    </div>`;
                criticalContainer.insertAdjacentHTML('beforeend', html);
            });
            $('.function-keys').select2({ tags: true });
        
            // TECHNICAL SKILLS
            const techContainer = document.getElementById('technicalSkillsContainer');
            techContainer.innerHTML = '';
            (jd.technicalSkills || []).forEach((item, index) => {
                const html = `
                    <div class="technical-skill row mb-8">
                        <div class="fv-row mb-2 fv-plugins-icon-container col-lg-9 d-flex gap-7">
                            ...
                            <select name="technicalSkills[${index}][id]" class="form-control select-techskill">
                                <option selected value="${item.id}">${item.name} (${item.sector_name}-${item.sub_sector_name})</option>
                            </select>
                        </div>
                        <div class="fv-row mb-2 fv-plugins-icon-container d-flex col-lg-3 btn-vl-container justify-content-end">
                            <select name="technicalSkills[${index}][level]" class="form-select">
                                ${[1, 2, 3, 4, 5, 6].map(l => `<option value="${l}" ${l === item.level ? 'selected' : ''}>Level ${l}</option>`).join('')}
                            </select>
                        </div>
                    </div>`;
                techContainer.insertAdjacentHTML('beforeend', html);
            });
            $('.select-techskill').select2({ placeholder: 'Search Skill', minimumInputLength: 1 });

    
            // Populate General Skills
            const genContainer = document.getElementById('generalSkillsContainer');
            jd.skills?.forEach((item, index) => {
                genContainer.innerHTML += `
                    <div class="technical-skill row mb-8">
                        <div class="fv-row mb-2 fv-plugins-icon-container col-lg-9 d-flex gap-7">
                            <select name="skills[${index}][title]" class="form-control mb-2" onchange="skillChanged(${index}, this.value)">
                                <option value="">Select Generic Skill</option>
                                ${masterSkills.map(skill => `<option value="${skill.name}" ${skill.name === item.title ? 'selected' : ''}>${skill.name}</option>`).join('')}
                            </select>
                        </div>
                        <div class="fv-row mb-2 fv-plugins-icon-container d-flex col-lg-3 btn-vl-container justify-content-end">
                            <select name="skills[${index}][level]" class="form-select">
                                ${[1,2,3].map(level => `<option value="${level}" ${level === item.level ? 'selected' : ''}>Level ${level}</option>`).join('')}
                            </select>
                            <button type="button" class="btn btn-outline btn-outline-primary btn-view d-flex justify-content-center rounded-1"
                                    data-bs-toggle="modal" data-bs-target="#genericSkillModal"
                                    onclick="populateModal('${index}', '...', '${item.title}', '${item.level}')">
                                View Level
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const jd = JSON.parse(data);
            console.log("Loaded JD from session:", jd);
            ...
            document.getElementById('level-job').value = jd.level || '';
            document.getElementById('level').value = jd.level || '';
            $('#riasec').val(jd.top3riasec?.split('') || []).trigger('change');
        
            // === Critical Work Functions ===
            const criticalContainer = document.getElementById('criticalFunctionsContainer');
            criticalContainer.innerHTML = '';
            if (Array.isArray(jd.criticalFunctions)) {
                jd.criticalFunctions.forEach((item, index) => {
                    const html = `
                        <div class="critical-function row mb-3">
                            <div class="col-lg-12">
            ...
            const techContainer = document.getElementById('technicalSkillsContainer');
            techContainer.innerHTML = '';
            if (Array.isArray(jd.technicalSkills)) {
                jd.technicalSkills.forEach((item, index) => {
                    const html = `
                        <div class="technical-skill row mb-8">
                            <div class="fv-row mb-2 fv-plugins-icon-container col-lg-9 d-flex gap-7">
            ...
            if (Array.isArray(jd.skills)) {
                jd.skills.forEach((item, index) => {
                    ...
                    genContainer.insertAdjacentHTML('beforeend', html);
                });
            }
        });
        </script>
