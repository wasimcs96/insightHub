...
@section('styles')
    <style>
        ...
        .modal-custom-check.custom-checkbox {
            display: flex;
            align-items: center;
            cursor: pointer;
        }

        .modal-custom-check.custom-checkbox input[type="checkbox"] {
            display: none;
            /* Hide the default checkbox */
        }

        .modal-custom-check.custom-checkbox .checkmark {
            width: 20px;
            height: 16px;
        ...
        .modal-custom-check.custom-checkbox input[type="checkbox"]:checked+.checkmark {
            background-color: #F7941C;
        ...
        .modal-custom-check.custom-checkbox .checkmark::after {
            content: "";
            position: absolute;
            left: 3px;
            top: 0px;
        ...
        .modal-custom-check.custom-checkbox input[type="checkbox"]:checked+.checkmark::after {
            opacity: 1;
        }
...
.select2-results__option:hover, .select2-container--bootstrap5 .select2-dropdown .select2-results__option.select2-results__option--selected, .select2-container--bootstrap5 .select2-dropdown .select2-results__option.select2-results__option--highlighted {
    background-color: #FFF6EA !important;
...
.dot-sky { background: #E3F7FF; }
.dot-yellow { background: #FFF5DA; }
.dot-green { background: #DDF5E2; }
.dot-purple { background: #F2EEFD; }
...
.btn:disabled, .add-function:disabled {
border: 1px solid #DBDFE9 !important;
...
.select2-search--dropdown::before { content: "\f002"; font-family: "Font Awesome 5 Free"; font-weight: 900; position: absolute; left: 10px; top: 50%; transform: translateY(-50%); color: #6c757d; pointer-events: none; }
...
@section('content')
    ...
    <div class="d-flex flex-column flex-column-fluid">
        @php
            ...
            $type = $cachedData['jd_type'] ?? null;


            // Dynamic page heading
            $pageHeading = 'Custom JD';
            $breadcrumbText = 'Custom JD';
            $heading = 'Custom JD';

            if ($type === 'ai') {
                $pageHeading = 'Match & Generate JD with AI';
                $breadcrumbText = 'Match & Generate JD with AI';
                $heading = 'Job Role Based on Master JD';
            } elseif ($type === 'company-jd') {
                $pageHeading = 'Based on Company JD';
                $breadcrumbText = 'Based on Company JD';
                $heading = 'Job Position Based on Company JD';
            } elseif ($type === 'master-jd') {
                $pageHeading = 'Based on Master JD';
                $breadcrumbText = 'Based on Master JD';
                $heading = 'Job Role Based on Master JD';
            }
            ...
            <div id="kt_app_toolbar_container" class="app-container  container-xxl d-flex flex-stack ">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 ">
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted" id="breadcrumbText">{{ $breadcrumbText }}</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div id="kt_app_content" class="app-content  flex-column-fluid ">
            ...
            <div id="kt_app_content_container" class="app-container  container-xxl">
                <form action="{{ route('admin.job-management.ai.store') }}" id="jd_create_form" method="post">
                    @csrf
                    {{-- <input type="hidden" name="jd_from" value="5"> --}}
                    @if ($type === 'ai')
                        <input type="hidden" name="jd_from" value="5">
                    @elseif ($type === 'master-jd')
                        <input type="hidden" name="jd_from" value="6">
                    @elseif ($type === 'company-jd')
                        <input type="hidden" name="jd_from" value="7">
                    @else
                        <input type="hidden" name="jd_from" value="8">
                    @endif
                    <input type="hidden" name="jobID" value="{{ $cachedData['job_id'] }}">
                    ...
                    <input type="hidden" name="localized_job" value="{{ $cachedData['is_localized_job'] ?? false }}">
                    <input type="hidden" name="oldJobId" value="{{ $oldJobId ?? null }}">
                    <input type="hidden" name="business_unit_id" value="{{ $cachedData['business_unit_id'] ?? '' }}">
                    <input type="hidden" name="sector_name" value="{{ $cachedData['sector_name'] ?? '' }}">
                    <input type="hidden" name="sub_sector_name" value="{{ $cachedData['sub_sector_name'] ?? '' }}">
                    <input type="hidden" name="sector" value="{{ $cachedData['sector'] ?? '' }}">
                    <input type="hidden" name="sierra_id" value="{{ $cachedData['cxs_job_table_id'] ?? $cachedData['sierra_id'] ?? '' }}">
                    <input type="hidden" name="ai_job_id" value="{{ $cachedData['ai_job_id'] ?? '' }}">
                    <input type="hidden" name="unique_id" value="{{ isset($cachedData['sierra_id']) ? $cachedData['sierra_id'] : '' }}">
                    ...
                    @php
                        ...
                        $jobFamilyTitle = Department::find($jobFamilyId)?->name ?? '';

                        $jobFamilyGroupId = $cachedData['job_family_group'] ?? null;
                        $jobFamilyGroupTitle = Division::find($jobFamilyGroupId)?->head_of_division ?? '';
                        $businessUnitTitle = BusinessUnit::find($cachedData['business_unit_id'] ?? null)?->name ?? '';

                    @endphp

                    <input type="hidden" name="status" value="2">
                    <input type="hidden" name="job_profile_name" value="{{ $jobProfileTitle ?? '' }}">
                    <input type="hidden" name="division_id" value="{{ $jobFamilyGroupId }}">
                    ...
                    @php
                        ...
                        // $heading = 'Job Role Based on Master JD';
                    @endphp
                    @if ($cachedData['jd_type'] == 'master-jd')
                    <div class="card  shadow-sm mb-4">
                        ...
                        <div class="form-group d-flex justify-content-evenly col-lg-12 p-0" style="gap: 16px">
                            {{-- If JD type is master-jd --}}
                                ...
                                <div class="col-lg-6">
                                    <label for="job_family_company" class="fw-semibold fs-7 mb-2">Related Job Role from
                                        Master JD</label>
                    ...
                    @endif
                    @if(in_array($cachedData['jd_type'] ?? null, ['company-jd', 'ai']))
                    <div class="card  shadow-sm mb-4">
                        <div class="bg-primary card-header">
                            <h3 class="card-title text-white">{{ $heading }}</h3>
                        ...
                        <div class="card-body  px-2 py-5">
                            ...
                            {{-- {{ dd($cachedData) }} --}}
                            <div class="form-group d-flex justify-content-evenly col-lg-12 p-0" style="gap: 16px">
                                @if (isset($cachedData['company_jd']) && $cachedData['company_jd'] == 0)
                                    <div class="w-100">
                                        <label for="job_family_group_company" class="fw-semibold fs-6 mb-2">Related Sector
                                            from Master JD</label>
                                    ...
                                    <div class="w-100">
                                        <label for="job_family_company" class="fw-semibold fs-6 mb-2">Related Job Role
                                            from
                                            Master JD</label>
                                ...
                                @else
                                    @php
                                        $jdLabelPrefix =
                                            $cachedData['jd_type'] == 'master-jd' ? 'Master JD' : 'Company JD';
                                    @endphp
                                    <div class="w-100">
                                        <label for="job_profile_company" class="fw-semibold fs-6 mb-2">Related Business Unit
                                            from {{ $jdLabelPrefix }}</label>
                                    ...
                                    <div class="w-100">
                                        <label for="job_family_group_company" class="fw-semibold fs-6 mb-2">Related Company/Division
                                            from {{ $jdLabelPrefix }}</label>
                    ...
                    <div class="card  shadow-sm mb-4">
                        ...
                        <div class="card-body  py-5">
                            ...
                            <div class="row">
<div class="fv-row fv-plugins-icon-container col-lg-12 d-flex existing_selects_sector_department" style="gap: 16px; margin-bottom: 24px;">
                                <div
                                    class="fv-row fv-plugins-icon-container existing_selects_sector_department w-100">
                                    <label for="business_unit_name" class="fw-semibold fs-6 mb-2 required">Business Unit</label>
                                    {{-- <select id="business_unit" name="business_unit_id" class="form-control" required readonly>
                                        <option value="">Select Business Unit</option>
                                        @foreach ($businessUnits as $unit)
                                            <option value="{{ $unit->id }}" @if ($cachedData['business_unit_id'] == $unit->id) selected @endif>{{ $unit->name }}</option>
                                        @endforeach
                                    </select> --}}
                                        <input id="business_unit_name" name="job_family_group" class="form-control"
                                            value="{{ $businessUnitTitle }}" readonly>
                                    </div>
                                    ...
                                    <div class="fv-row  fv-plugins-icon-container w-100">
                                        ...
                                        <input id="job_family_group" name="job_family_group" class="form-control"
                                            value="{{ $jobFamilyGroupTitle }}" readonly>
                                ...
                                <div class="fv-row  fv-plugins-icon-container col-lg-12" style="margin-bottom: 24px;">
                                    ...
                                    <div id="jobPositionValidationMessage" class="text-danger validation-message" style="display: none;">
                                        Job Position is required
                                    </div>


                                    <div class="custom-toggle-wrapper" style="margin-top: 8px;">
                                        @if (!isset($cachedData['is_localized_job']))
                                ...
                                <div style="margin-bottom: 24px;display: flex;gap: 19.5px;">
                                    <div class="fv-row  fv-plugins-icon-container w-100">
                                    ...
                                    <div class="fv-row fv-plugins-icon-container w-100">
                                        <div class="d-flex gap-2 align-items-center"><label for="superior" class="fw-semibold fs-6 mb-2">Superior Job Position</label><div class="d-flex align-items-center gap-1 mb-2 d-none" id="superior-warning" style="color: #4B5675;"><iconify-icon icon="mingcute:warning-line" width="16" height="16" style="color: #F7941C;"></iconify-icon> The selected superior is from a different department.</div></div>
                                        ...
                                        <div id="superiorValidationMessage" class="text-danger validation-message" style="display: none;">
                            ...
                            <div class="row" style="margin-bottom: 24px;">
                                <div class="fv-row fv-plugins-icon-container col-lg-6">
                                    ...
                                    <select id="level-job" class="form-control" name="level"
                                        onchange="updatePositionCode()">
                                    ...
                                    <div id="levelJobValidationMessage" class="text-danger validation-message" style="display: none;">
                                        Please select a position level
                                    </div>
                                </div>
                                <div class="fv-row  fv-plugins-icon-container col-lg-6">
                                    <label for="position_level" class="fw-semibold fs-6 mb-2">Management
                                        Level <span class="text-muted"><em>-Optional</em></span></label>

                                    <select id="level-job" class="form-control form-control mb-lg-0" name="level_job"
                            ...
                            <div class="mb-5">
                                <label for="" class="form-label">
                                    @if (!in_array($type, ['ai', 'company-jd', 'master-jd']))
                                        Current Job Description for Custom JD
                                    @else
                                        Job Role Description (Retrieved from Master List)
                                    @endif
                                </label>
                            ...
                            {{-- @if ($type === 'ai' || $type === 'company-jd' || $type === 'master-jd')
                                
                            @if (!isset($cachedData['source']) || $cachedData['source'] !== 'formatJobsbyfamily') --}}
                            @if (isset($cachedData['job_type']))
                            @if (!isset($cachedData['job_type']) || ($cachedData['job_type'] === 'ai' || $cachedData['job_type'] === 'master-jd'))
                    ...
                    <x-headcount-management :headcounts="$headcounts" :topJobId="$topJobId" :topJobTitle="$topJobTitle" :isTopExists="$isTopExists"
                        :topJob="$topJob" :cachedData="$cachedData" />
                        ...
                        <div class="card-body">
                            <div class="fv-row fv-plugins-icon-container col-lg-12 d-flex p-0"
                                ...
                                <div class="input-area w-100">
                                    ...
                                    <div id="educationLevelValidationMessage" class="text-danger validation-message" style="display: none;">
                                ...
                                <div class="input-area w-100">
                                    ...
                                    {{-- <select id="secondary_scope_of_study" class="form-select mb-3 mb-lg-0"
                                        ...
                                        <option value="" class="dark:bg-slate-700">Select Secondary Scope of Study
                                        </option>

                                    </select> --}}

                                    <input class="form-control" placeholder="Secondary Scope of Study" type="text" id="kt_tagify_5"
                                    value=""
                                    name="secondary_scope_of_study"  />
                            ...
                            <div class="fv-row fv-plugins-icon-container col-lg-12 d-flex p-0" style="gap: 16px;">
                                <div class="input-area w-100">
                                    <div>
                                        ...
                                        <input class="form-control" placeholder="Relevant Professional Certificates"
                                            type="text" value="" id="kt_tagify_1"
                                            name="professional_certificate" id="kt_tagify_1" />
                                    </div>
                                </div>
                                ...
                                <div class="input-area w-100">
                                    <div>
                                        ...
                                        <input class="form-control" value=""
                                            placeholder="Relevant Training Programs" name="relevant_training"
                                ...
                                <div class="input-area w-100">
                                    ...
                                    <option value="" disabled selected>Select Experience in Relevant Sector</option>
                    ...
                    <div class="card  shadow-sm mb-4">
                        ...
                        <div class="card-body  py-5">
                            ...
                            <button type="button" id="add_new_function" class="add-function mt-5"
                                ...
                                Function</button>
                            <div id="validationMessage" class="mt-3" style="color: red; display: none;">At least one Critical Work
                                Function
                                is required.</div>
                      ...
                      <x-riasec-selector 
                        :selectedRiasec="str_split($cachedData['top3riasec'] ?? '')"
                        :masterId="$cachedData['cxs_job_table_id'] ?? null"
                        :currentTop3Riasec="$cachedData['top3riasec'] ?? ''"
                        jobProfileFieldId="job_profile"
                        jobDescriptionFieldId="jobRoleDescription"
                        :jdType="$type"
                        :jdTitle="$cachedData['title'] ?? ''"
                      />
                    ...
                    <div class="card  shadow-sm mb-4">
                        <div class="card-header d-flex" style="justify-content: space-between !important;">
                             <h3 class="card-title"> Technical Skills <iconify-icon icon="material-symbols:info-outline-rounded" data-bs-toggle="tooltip" class="mx-2 info-tooltip" data-bs-placement="top" data-bs-html="true"
  data-bs-title='<div class="custom-tooltip"><span><span class="dot dot-sky"></span> Master Skill</span><span><span class="dot dot-yellow"></span> Company Skill</span><br/><span><span class="dot dot-green"></span> Sector</span><span><span class="dot dot-purple"></span> Category</span></div>' width="20" height="20" style="color: #5F6368;"></iconify-icon></h3>
                        ...
                        <div class="card-body  py-5">
                            ...
                            <button type="button" id="add_new_skill" class="add-function mt-5"
                                style="background:#F7941C"><iconify-icon icon="stash:plus-solid"
                                    style="font-size: 20px"></iconify-icon> Add New Skill</button>
                            <div id="techvalidationmessage" class="mt-3" style="color: red; display: none;">At least One Technical Skill
                                is
                                required.</div>
                        </div>
                    ...
                    <div class="card  shadow-sm mb-4">
                        ...
                        <div class="card-body  py-5">
                            ...
                            <div class="col-lg-4">
                                ...
                                <select class="form-select" data-control="select2" name="status" data-placeholder="Select Status">
                                    <option></option>
                            ...
                            <div class="mb-0 d-flex col-lg-12 p-0">
                                ...
                                <div class="d-flex align-items-center gap-3 feedback-btn" style="color: #3E3E3E;">
                                    ...
                                    <div class="thumbs-down" data-bs-toggle="modal" data-bs-target="#feedbackModal"
                                        id="thumbs-down">
                                        <iconify-icon data-bs-toggle="modal" data-bs-target="#feedbackModal"
                                            id="thumbs-down" icon="ri:thumb-down-line" class="cursor-pointer"
                                            width="26" height="26" data-bs-toggle="tooltip"
                                            data-bs-placement="top" title="Bad Result"></iconify-icon>
                                    </div>
                    ...
                    @if (($cachedData['job_type'] ?? null) === 'ai')
                    <div class="card  shadow-sm mb-11">
                        <div class="card-header">
                            <h3 class="card-title">Getting feedback <span class="text-muted"> <em>-optional</em></span>
                            </h3>
                        </div>
                        <div class="card-body  py-5">
                            <div class="d-flex gap-3 fs-5"><span>How's the overall result generated by the AI?</span>
                                <span data-bs-toggle="modal" data-bs-target="#feedbackModal" id="thumbs-up" class="thumb-btn"><iconify-icon
                                        icon="codicon:thumbsup" width="24" height="24" class="cursor-pointer"
                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                        data-bs-title="Good Result"></iconify-icon></span>
                                <span data-bs-toggle="modal" data-bs-target="#feedbackModal"
                                    id="thumbs-down" class="thumb-btn"><iconify-icon icon="codicon:thumbsdown" width="24"
                                        height="24" data-bs-toggle="tooltip" data-bs-placement="top"
                                        data-bs-title="Bad Result" class="cursor-pointer"></iconify-icon></span>
                            </div>
                        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body py-0 pt-5 text-center">
                    ...
                    <h4 class="fw-bold lh-1 text-center" style="font-size: 33.5px;">Create New Company<br> Technical Skill?</h4>
                    <p class="m-0 text-center mt-6">This action will create a new company technical skill from the selected master skill.</p>
                </div>
                <div class="modal-footer modal-footer d-block border-0">
                <div class="filter-content d-flex justify-content-center gap-2">
                <button  class="btn btn-outline m-0" style="color: #78829D;" data-bs-dismiss="modal">Cancel</button>
                    <button id="continueEditSkillBtn" class="btn btn-apply text-white continueEditSkillBtn" style="background: #F7941C;">
                        Create
                    </button>
                </div>
                </div>
    ...
    <div class="modal fade" id="CreateNewSkillModal" tabindex="-1" aria-labelledby="CreateNewSkillModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body py-0 pt-5 text-center">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="float: right;"></button>
                    <iconify-icon icon="ep:warning" width="70" height="70" class="my-5"
                        style="color: #FABB6E;"></iconify-icon>
                    <h4 class="fw-bold lh-1 text-center" style="font-size: 33.5px;">Create Another New<br> Company Technical Skill?</h4>
                    <p class="m-0 text-center mt-6">There are already company technical skill(s) created based on this master skill. Are you sure you want to create another one?</p>
                    
                </div>
                <div class="modal-footer modal-footer d-block border-0">
                <div class="filter-content d-flex justify-content-center gap-2">
                <button  class="btn btn-outline m-0" style="color: #78829D;" data-bs-dismiss="modal">Cancel</button>
                    <button id="" class="btn btn-apply text-white continueEditSkillBtn" style="background: #F7941C;">
                        Create Another
                    </button>
                </div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body py-0 pt-5 text-center">
                    ...
                    <p class="m-0 text-center mt-6">You haven’t made any changes to this skill. If you continue, no new company technical skill will be created and it will remain as a master skill.</p>
    ...
    <div class="modal fade" id="NoChangesCompanyModal" tabindex="-1" aria-labelledby="NoChangesModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body py-0 pt-5 text-center">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="float: right;"></button>
                    <iconify-icon icon="ep:warning" width="70" height="70" class="my-5"
                        style="color: #FABB6E;"></iconify-icon>
                    <h4 class="fw-bold lh-1 text-center" style="font-size: 33.5px;">No Changes Detected</h4>
                    <p class="m-0 text-center mt-6">You haven’t made any changes to this skill. If you continue, no new company technical skill will be created.</p>
    ...
    <div class="modal fade" id="SuccessModal" tabindex="-1" aria-labelledby="SuccessModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body py-0 pt-5 text-center">
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="float: right;"></button>
                    <iconify-icon icon="simple-line-icons:check" width="70" height="70" class="my-5" style="color: #99E2A8;"></iconify-icon>
                    <h4 class="fw-bold lh-1 text-center" style="font-size: 33.5px;">Company Skill Created</h4>
                    <p class="m-0 text-center mt-6">The new company technical skill, <b id="SkillName">Skill Name</b>, has been created successfully.</p>
                </div>
                <div class="modal-footer modal-footer d-block border-0">
                <div class="filter-content d-flex justify-content-center gap-2">
                <button type="button" class="btn btn-outline" data-bs-dismiss="modal" style="background-color: #F7941C; color: #fff;">Close</button>
                </div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                        ...
                        <form>
                            <div class="d-flex align-items-stretch justify-content-center flex-column">
                                <div class="d-flex flex-column gap-3 border manually-modal-inner h-100">
                                    <p class="m-0 text-left fw-bolder fs-2">Job Description (Retrieved from Master List)
                                    </p>
                                    <div class="line"></div>
                                ...
                                <h4 class="modal-jd-heading fs-1 fw-bolder m-0" id="jd_job_profile">Cabin Crew 2</h4>
                                <div class="mt-5">
                                    <label for="additionalDetail" class="form-label fw-semibold text-dark">Job Description
                                        from {{ env('APP_NAME') }}</label>
                    ...
                    <div id="step2" class="d-none">
                        <h4 class="modal-jd-heading-generate fs-1 fw-bolder d-flex align-items-center gap-3">Localise JD
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="custom-popup-body modal-content">
                ...
                <div class="modal-body pb-0">
                    ...
                    <div class="m-auto">
                        <iconify-icon icon="jam:alert" class="text-center d-flex justify-content-center" width="70" height="70"
                            ...
                            <div class="d-flex align-items-center justify-content-center gap-4 mb-5">
                                <label class="manually-radio w-50">
                                    ...
                                    <div class="d-flex flex-column gap-3 manually-modal-inner" style="height: 160px;">
                                        ...
                                        <div class="line"></div>
                                        <p class="m-0 text-center">Customise and create as new company technical skill specifically for this JD and to save it in the Company Technical Skills Library.</p>
                                    </div>
                                </label>
                                <label class="manually-radio w-50">
                                    <input type="radio" name="jdOption" value="master" id="masterJD">
                                    <div class="d-flex flex-column gap-3 manually-modal-inner" style="height: 160px;">
                                        ...
                                        <div class="line"></div>
                                        <p class="m-0 text-center">Replace the existing skill in the Company Technical Skill Library with the updated version. This will affect <b id="jobCount">7</b> <b>JDs</b> currently linked to it.</p>
                                    </div>
                                </label>
                            </div>
                        </form>
...
try {
    ...
    <script>
        ...
        // IMPORTANT: This is OUTSIDE $(document).ready() to ensure it only runs ONCE
        // Using namespaced event to prevent duplicates
        (function() {
            if (window.__removeSkillHandlerAttached) return; // Prevent multiple attachments
            window.__removeSkillHandlerAttached = true;
            ...
            $(document).off('click.removeSkillHandler', '.remove-skill').on('click.removeSkillHandler', '.remove-skill', function () {
                ...
                ModalManager.open({
                    ...
                    onCancel(modalEl) {
                        // User cancelled - just hide overlay
                        try { if (typeof hideOverlay === 'function') hideOverlay(); } catch (e) {}
                    },
                    onShown(modalEl) {
        ...
        // ===== End global delegated event handler =====
    ...
    if (techCount > 0) { $('#techvalidationmessage').hide(); } else { $('#techvalidationmessage').show(); }

    // GENERIC SKILLS
    // const genericCount = $('#generic-skills-container .generic-skill').length;
    // if (genericCount >= 3) { $('#genericValidationMessage').hide(); } else { $('#genericValidationMessage').show(); }
    const genericCount = $('#generic-skills-container .generic-skill').length;
    ...
    if (cwfCount > 0) { $('#validationMessage').hide(); } else { $('#validationMessage').show(); }
  }
  ...
  const observeChildren = (selector) => {
    ...
    if (!node) return;
    new MutationObserver(updateSectionWarnings).observe(node, { childList: true });
  };
  ...
  // When removing rows
  $(document).on('click', '.remove-technical-skill, .remove-generic-skill, .remove-critical-function', updateSectionWarnings);
...
if (unsavedCWFs.length > 0) {
        ...
        if (isTitleEmpty && areAllTasksEmpty) {
            ...
            if (isTitleEmpty) {
                ...
                if ($functionInput.next('.function-error').length === 0) {
                    $functionInput.after('<div class="function-error text-danger small">This field is required</div>');
                }
            } else {
            ...
            $taskInputs.each(function () {
                ...
                if (value === '') {
                    ...
                    if ($task.next('.task-error').length === 0) {
                        $task.after('<div class="task-error text-danger small">This field is required</div>');
                    }
                } else {
    ...
    const index = $button.attr('id').replace('selectTechLevelButton', '');  // Extract index from the button's id
    const $select = $(`#skill\\[${index}\\]`);  // Get the corresponding select element by index
    ...
    if ($button.closest('.technical-skill').length) {
        ...
        if (levelText === "Select Level" || levelText === "Level" || !levelValue) {
            ...
            $select.addClass('is-invalid');  // Add invalid class to the select element
            ...
            if ($button.length && $button.next('.level-validation-message').length === 0) {
                ...
                `);
                $button.addClass('border-danger text-danger');  // Add red border to the button
            }
        } else {
            // Reset validation if the level is properly selected
            $select.removeClass('is-invalid');
            $button.removeClass('border-danger text-danger');
            $button.next('.level-validation-message').remove();  // Remove the error message
        }
    }
});
   ...
   $('.technical-skill').each(function () {
    ...
    // --------------------------------------------
    console.log('genericSkillLength', $('#generic-skills-container .generic-skill').length);
    if ($('#generic-skills-container .generic-skill').length < 3) {
        ...
        $('#generic-skills-container .generic-skill').each(function () {
            ...
            if (!preferredLevel || preferredLevel === '') {
                ...
                $button.addClass('border-danger text-danger');
                $button.after(`<div class="level-validation-message text-danger mt-1" style="font-size: 0.875rem;">At least one level is required</div>`);
            }
        });
    }
    ...
    //     $('#headcount').addClass('is-invalid');
    //     $('#headValidationMessage').show();
    //     isValid = false;
    // }
...
// }
        ...
        $(document).ready(function() {
            ...
            $('#job_description_form').submit(function(event) {
                ...
                if (job_role === '') {
                    ...
                    $('#job_role').after('<span class="error">Job Position is required</span>');
                    isValid = false;
                } else {
                    $('#job_role').next('.error').remove();
                }
    ...
    <script>
        ...
        //     console.log('selectedLevelllllllllllll', selectedLevel);
        ...
        //         existingInput.val(selectedLevel);
        //     } else {
        //         container.prepend(
        //             `<input type="hidden" name="genericSkills[${index}][preferred_level]" value="${selectedLevel}">`
        //         );
        //     }
    ...
    <script>
        ...
        function populateForm(data) {
            ...
            document.querySelector('textarea[name="description"]').value = data.description || data.job_profile_description || '';
        ...
        `);
            ...
            // DUPLICATE HANDLER - Commented out because global delegated handler now handles all clicks
...
//             $('form').on('submit', function(e) {
//                 // If modal already confirmed, allow form to submit normally
//                 if (modalConfirmed) return true;

//                 e.preventDefault(); // prevent first submission

//                 const skillCount = $('.technical-skill').length;
//                 const riasecSelected = $('#riasec').val()?.length >= 3;
...
//                     $('#riasec').addClass('is-invalid');
//                     $('html, body').animate({
//                         scrollTop: $('#riasec-block').offset().top - 100
//                     }, 500);
//                     toastr.error("Please fill RIASEC code", "Validation Error", {
//                         closeButton: true,
//                         progressBar: true,
//                         timeOut: 4000
//                     });
...
//                     }, 500);
//                     toastr.error("At least one Technical Skill is required.", "Validation Error", {
//                         closeButton: true,
//                         progressBar: true,
//                         timeOut: 4000
//                     });
...
//                 // ModalManager.open({
//                 //     module: 'jobs',
//                 //     key: "jd_create_redirect",
//                 //     data: {},
//                 //     onShown(modalEl) {
//                 //         // $(modalEl).find('#btn-create-page, #btn-detail-page').on('click',
//                 //         //     function() {
//                 //         //         const target = $(this).data('target'); // 'create' or 'detail'
//                 //         //         $('#redirect_target').val(target);
...
//                 // const jobProfileValue = document.getElementById('job_profile')?.value.trim();
...
//                 //         data: {},
//                 //         onShown(modalEl) {
//                 //             // Optional: logic when modal is shown
//                 //         },
//                 //         onSubmit(modalEl1) {
//                 //             modalConfirmed = true;
//                 //             $('#jd_create_form').submit();
...
//                 //     });
//                 // } else {
//                 //     console.log('Job profile is empty. ModalManager will not be triggered.');
//                 // }

//                 // const jobProfileValue = document.getElementById('job_profile')?.value.trim();
//                 // const positionLevel = document.getElementById('level-job')?.value.trim();
//                 // const educationLevel = document.getElementById('education_level')?.value.trim();
//                 // const riasecSelectedOptions = $('#riasec').val() || [];

//                 // if (
//                 //     jobProfileValue &&
//                 //     positionLevel &&
//                 //     educationLevel &&
//                 //     riasecSelectedOptions.length >= 3
//                 // ) {
//                 //     ModalManager.open({
//                 //         module: 'jobs',
//                 //         key: "jd_create_redirect",
...
//                 //     });
//                 // } else {
//                 //     console.log('One or more required fields are empty or incomplete. ModalManager will not be triggered.');
//                 // }
...
// //         if (!levelSelected || !levelValue) {
// //             isValid = false;
// //         }
// //     });

// //     $('.technical-skill').each(function () {
// //         const skillVal = $(this).find('.select-skill').val();
// //         if (!skillVal) {
// //             isValid = false;
// //         }
// //     });
...
//     // Loop through each edit-level-btn (i.e., button for technical skills)
//     $('button.edit-level-btn').each(function () {
//         const $button = $(this);
//         const levelText = $button.text().trim();  // Get the button text (Level text)
//         const index = $button.attr('id').replace('selectTechLevelButton', '');  // Extract index from the button's id
...
//         // Check if this button belongs to a technical skill (ensure it's inside .technical-skill container)
//         if ($button.closest('.technical-skill').length) {
//             // Check if the level text is "Select Level", "Level", or if no level is selected
//             if (levelText === "Select Level" || levelText === "Level" || !levelValue) {
//                 isValid = false;  // Mark the level as invalid
//                 $select.addClass('is-invalid');  // Add invalid class to the select element
...
//     // Loop through technical skills (to check if any skill is selected)
//     $('.technical-skill').each(function () {
//         const skillVal = $(this).find('.select-skill').val();
//         if (!skillVal) {
//             isValid = false;  // If no skill is selected, mark as invalid
//         }
//     });

//     return isValid;  // Return the result of technical skills validation (true or false)
// }


// function validateGenericSkills() {
//     let isValid = true;

//     const $genericSkills = $('#generic-skills-container .generic-skill');
//     if ($genericSkills.length < 3) {
//         isValid = false;
//     } else {
//         $genericSkills.each(function () {
//             const skillIndex = $(this).attr('id').split('-').pop();
//             const preferredLevel = genericSkillsData[skillIndex]?.preferred_level;
...
// // function validateCriticalWorkFunctions() {
// //     let isValid = true;
// //     const unsavedCWFs = $('.critical-function[data-has-saved="false"]');

// //     unsavedCWFs.each(function () {
// //         const $wrapper = $(this);
// //         const titleVal = $wrapper.find('.function-input').val().trim();
// //         const taskVals = $wrapper.find('.task-input').map(function () {
// //             return $(this).val().trim();
// //         }).get();
...
//      let hasJobProfile = true;
//     let hasPositionLevel = true;
//     // const jobProfileValue = document.getElementById('job_profile')?.value.trim();
//     // const positionLevel = document.getElementById('level-job')?.value.trim();
//      if (!$('#job_profile').prop('disabled')) {
//         const jobProfileValue = $('#job_profile').val()?.trim();
//         hasJobProfile = !!jobProfileValue;
//         if (!hasJobProfile) {
//             console.log('Job profile is empty');
//         }
//     }

//     if (!$('#level-job').prop('disabled')) {
//         const positionLevel = $('#level-job').val()?.trim();
//         hasPositionLevel = !!positionLevel;
//         if (!hasPositionLevel) {
//             console.log('Position level is empty');
//         }
//     }
//     const educationLevel = document.getElementById('education_level')?.value.trim();
//     const riasecSelectedOptions = $('#riasec').val() || [];
...
//     const hasMinimumRiasec = riasecSelectedOptions.length >= 3;

//     const skillsValid = validateTechnicalSkills();
//     const genericValid = validateGenericSkills();
//     // const cwfValid = validateCriticalWorkFunctions();
...
// // Trigger modal only if everything is valid
...
//     });
// } else {
//     console.log('ModalManager not triggered: One or more fields are incomplete or invalid.');
// }
...
(function ($) {
  // ========= CONFIG =========
  const FORM_SEL = 'form#jd_create_form';            // adjust if your form has another selector
  ...
  const UNSAVED_MODAL_ID = 'UnsavedTechChangesModal'; // the “Unsaved Changes Detected” modal id

  let modalConfirmed = false; // set true only when user confirms the final redirect modal

  // === helpers ===========================================================
  const normalize = (s) => String(s ?? '').replace(/\s*\([^)]*-[^)]*\)\s*/g, '').trim();

  // Seed data-original on all current tech fields so we can compare later
  function seedOriginals(root = document) {
    $(root).find(`${TECH_CONTAINER} ${TECH_ROW}`).each(function () {
      $(this).find(TECH_FIELDS).each(function () {
  ...
  // true if any visible tech row is in edit mode or has changed values
  function hasUnsavedTech() {
    let unsaved = false;
    const $rows = $(`${TECH_CONTAINER} ${TECH_ROW}:visible`);

    $rows.each(function () {
      if (unsaved) return false;
      const $row = $(this);

      // if an Accept button is visible, we consider the row “editing”
      if ($row.find('.accept-btn:visible').length > 0) {
        unsaved = true;
        return false;
      }

      // compare visible fields vs their originals
      $row.find(`${TECH_FIELDS}:visible`).each(function () {
        const orig = $(this).data('original') ?? '';
        const curr = $(this).val() ?? '';
        if (normalize(orig) !== normalize(curr)) {
          unsaved = true;
          return false;
        }
      });
  ...
  // keep row “dirty” marker in sync as user edits (purely visual/optional)
  $(document).on('input change', `${TECH_CONTAINER} ${TECH_ROW} ${TECH_FIELDS}`, function () {
    const $row = $(this).closest(TECH_ROW);
    let dirty = false;
    $row.find(TECH_FIELDS).each(function () {
      const orig = $(this).data('original') ?? '';
      const curr = $(this).val() ?? '';
      if (normalize(orig) !== normalize(curr)) { dirty = true; return false; }
    });
    if (dirty) $row.attr('data-dirty', '1'); else $row.removeAttr('data-dirty');
  });

  // Commit changes when user clicks Accept (update originals)
  $(document).on('click', `${TECH_CONTAINER} ${TECH_ROW} .accept-btn`, function () {
    const $row = $(this).closest(TECH_ROW);
    $row.find(TECH_FIELDS).each(function () {
      $(this).data('original', $(this).val() || '');
    });
    $row.removeAttr('data-dirty');
  });

  // Re-seed originals for dynamically added skill rows
  const techNode = document.querySelector(TECH_CONTAINER);
  if (techNode) {
    new MutationObserver((mut) => {
      mut.forEach(m => m.addedNodes?.forEach(node => {
        if (!(node instanceof HTMLElement)) return;
        if (node.matches?.(TECH_ROW) || node.querySelector?.(TECH_ROW)) {
          seedOriginals(node);
        }
      }));
    }).observe(techNode, { childList: true, subtree: true });
  }

  // === your validations kept simple here (keep/extend as needed) =========
  function validateRIASEC() {
    const arr = $('#riasec').val() || [];
    if (arr.length >= 3) {
      $('#riasec-error').hide(); $('#riasec').removeClass('is-invalid');
      return true;
    }
    $('#riasec-error').show(); $('#riasec').addClass('is-invalid');
    $('html, body').animate({ scrollTop: $('#riasec-block').offset().top - 100 }, 500);
    toastr.error("Please fill RIASEC code", "Validation Error", { closeButton:true, progressBar:true, timeOut:4000 });
    return false;
  }

  function validateTechnicalSkillsPresence() {
    const count = $(`${TECH_CONTAINER} ${TECH_ROW}`).length;
    if (count > 0) {
      $('#techvalidationmessage').hide(); return true;
    }
    $('#techvalidationmessage').show();
    $('html, body').animate({ scrollTop: $('#techvalidationmessage').offset().top - 100 }, 500);
    toastr.error("At least one Technical Skill is required.", "Validation Error", { closeButton:true, progressBar:true, timeOut:4000 });
    return false;
  }

  // If you also need to validate levels etc, keep your existing functions and call them here.

  function allGoodForRedirectModal() {
    // example: job_profile, level-job, education_level must be filled
    const jobOk  = $('#job_profile').prop('disabled') ? true : !!($('#job_profile').val() || '').trim();
    const lvlOk  = $('#level-job').prop('disabled') ? true : !!($('#level-job').val() || '').trim();
    const eduOk  = !!($('#education_level').val() || '').trim();
    const riasecOk = validateRIASEC();
    const techOk   = validateTechnicalSkillsPresence();
    // Add your detailed technical/generic validations here if you need:
    // const techDetailOk = validateTechnicalSkills(); const genericOk = validateGenericSkills();

    return jobOk && lvlOk && eduOk && riasecOk && techOk; // && techDetailOk && genericOk
  }

  // === submit handler =====================================================
  $(document).on('submit', FORM_SEL, function (e) {
    if (modalConfirmed) return true; // already confirmed in redirect modal

    e.preventDefault(); // stop the first submit

    // 1) run the simple validations
    if (!validateRIASEC() || !validateTechnicalSkillsPresence()) return false;

    // 2) make sure originals exist before checking unsaved
    seedOriginals();

    // 3) if there are unsaved Technical Skills → show “Unsaved Changes” modal and STOP
    if (hasUnsavedTech()) {
      const dialog = document.getElementById(UNSAVED_MODAL_ID);
      bootstrap.Modal.getOrCreateInstance(dialog).show();
      return false;
    }

    // 4) otherwise, open your redirect/confirmation modal; on confirm, submit for real
    if (allGoodForRedirectModal()) {
      ModalManager.open({
        module: 'jobs',
        key: "jd_create_redirect",
        data: {},
        onShown(modEl) { /* optional */ },
        onSubmit(modEl) {
          modalConfirmed = true;          // allow next submit through
          $(FORM_SEL)[0].submit();         // submit programmatically
        }
      });
    }
    return false;
  ...
  // initial seeding once DOM is ready
  $(function(){ seedOriginals(); });
        ...
        //                 return {
        //                     q: params.term,
        //                     page: params.page || 1 // ✅ Add this line
        //                 };
        //             },
        //             processResults: function(data, params) {
        //                 params.page = params.page || 1;
        ...
        //                             .more // ✅ Allows Select2 to know whether to fetch next page
        //                     }
        //                 };
        //             },
        //             cache: true
    ...
    function renderSkillMarkup(data) {
        ...
        return $(`
            <div class="d-flex justify-content-between align-items-center w-100 gap-6">
                <span class="span-truncate" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${name}" data-skillType="${skillType}">
                    ${name}
    ...
    // ✅ Re-init tooltips when dropdown opens or selection changes
...
function removeSkillFromData(index) {
    ...
    `);
            ...
            //             return {
            //                 q: params.term,
            //                 page: params.page || 1 // ✅ Fix: required for pagination
            //             };
            //         },
            //         processResults: function(data, params) {
            //             params.page = params.page || 1;
    ...
    templateSelection: function (data) {
        ...
        return $(`
            <div class="d-flex justify-content-between align-items-center w-100 gap-6">
                <span class="span-truncate" data-bs-toggle="tooltip" data-bs-title="${name}" data-skillType="${skillType}">
                    ${name}
                </span>
                <div class="d-flex gap-2">
                    ${skillType 
    ...
    width: 'resolve'
            ...
            //     // Update hidden input
            //     $(`#technicalSkillsHidden\\[${index}\\]`).val(skillName);
            //     TechSkillChanged(index, id, $(this));
            //     // Store in global data
            //     technicalSkillsData[index] = {
            //         id: id,
            //         name: skillName,
            ...
            //     };
            // });
             // Global guard to prevent infinite loops
const _changingGuard = new WeakMap();

skillSelect.off('change.dupcheck').on('change.dupcheck', function () {
    const $sel = $(this);

    // Skip handler if we're in a programmatic reset
    ...
    // --- Duplicate check across all selects ---
    let isDuplicate = false;
    $('.select-skill').each(function () {
        const otherIndex = $(this).data('index');
        const otherValue = $(this).val();
        ...
        // Skip self
        if (Number(otherIndex) !== Number(skillIndex) && otherValue === id) {
            isDuplicate = true;
            return false; // break
        }
    });
    ...
    if (isDuplicate) {
        // Temporarily disable handler recursion
        _changingGuard.set(this, true);
        $sel.val(null).trigger('change.select2'); // triggers only Select2 UI refresh
    ...
    // Update hidden field safely (brackets must be escaped)
    $(`#technicalSkillsHidden\\[${skillIndex}\\]`).val(skillName);

    TechSkillChanged(skillIndex, id, $sel);

    // Update technicalSkillsData store
    technicalSkillsData[skillIndex] = {
        ...
        is_custom: skillType
            ...
            $(document).off('click', '.edit-skill').on('click', '.edit-skill', function () {
                ...
                console.log("Editing skill at index:", skillIndex, skillData);
                ...
                if (!skillData) {
                    ...
                    console.log("adafsd", selectedOption.text().includes('Company Skill'));
                ...
                //  let selectedOption =$(`#skill\\[${skillIndex}\\] option:selected`);
                 ...
                 console.log("Cleaned Text:", selectedOption.text());
                console.log("Child elements:", selectedOption.attr('skillType'));
        ...
        function technicalpopulateModal(index) {
            ...
            $('#techskillmodal .save-btn').off('click').on('click', function() {
                ...
                if (inputEl.length) {
                    ...
                    // ✅ Update data-original to prevent "unsaved changes" detection
                    inputEl.data('original', selected);
                } else {
                    const newInput = $(`
                        <input type="hidden" name="technicalSkills[${skillIndex}][preferred_level]" value="${selected}">
                    `);
                    $(`#skill-${skillIndex}`).prepend(newInput);
                    // ✅ Set data-original on the newly created input
                    newInput.data('original', selected);
                }
                
                // ✅ Update data-original on all technical skill fields for this row to mark as "saved"
                $(`#skill-${skillIndex}`).find('input[name^="technicalSkills"], textarea[name^="technicalSkills"]').each(function() {
                    $(this).data('original', $(this).val() || '');
                });
                
                // ✅ Remove dirty marker if present
                $(`#skill-${skillIndex}`).removeAttr('data-dirty');
    ...
    $('#genericSkillModal .save-btn').prop('disabled', false);
        ...
        function toggleRemoveSkillButtons() {
            ...
            // Exclude removed skills from count - only count active technical skills
            ...
            // Only target remove buttons in active (non-removed) technical skills
            ...
            // Only disable if we have exactly 1 active technical skill
        ...
        function addGenericSkill(index, skill = null, allSkills = []) {
            if (!skill) return;
            const GenpreferedLvlLabel = skill?.preferred_level;
            const GenpreferedLvl = getPreferredLevel(skill?.preferred_level)
            ...
            console.log('tttttttttttttttttttttttttttttttttttttttttttttttttt', genericSkillsData[index]);
  ...
  document.getElementById('proceedCompanySkillBtn')
    ...
    <script>
        ...
        function updatePositionCode() {
            ...
            const positionCodeInput = document.getElementById('job_profile_id');
    ...
    <script>
        $(document).ready(function() {
            // Old RIASEC handlers removed - now handled by x-riasec-selector component
        });
    </script>
...
function attachSkillRowEventListeners(skillIndex) {
  // DUPLICATE HANDLER - Function body emptied because global delegated handler now handles all clicks
  // const skillRow = document.getElementById(`skill-${skillIndex}`);
  // if (!skillRow) return;
  // 
  // // Attach remove button event listener with ModalManager
...
if (_m) _m.hide();
  ...
  return (
        ...
        btn.onclick = () => {
            ...
            document.querySelectorAll('.add-level-btn').forEach(btn => {
                btn.onclick = () => {
                    ...
                    skill[`level_${level}_knowledge`] = ['']; // no rows
                    skill[`level_${level}_ability`] = ['']; 
    ...
    if (jsonInput && jsonInput.value) {
      ...
      } catch (e) { console.warn('Failed to parse #technicalSkillsJson'); }
    }
    let hasAttemptedSave = false;
            window.saveTSEditLevelModal = function(e) {
                ...
                allLevelBoxes.forEach(box => {
                    ...
                    knowledgeFields.forEach(textarea => {
                        const val = textarea.value.trim();
                        const errorDiv = textarea.closest('.error-div'); // 👈 target error-div wrapper
    ...
    closeBtn.addEventListener('click', function () {
      ...
      if (didSave) { didSave = false; return; } // user saved → do not restore
...
$(document).on('click', '.cancel-or-delete-function', function () {
    ...
    <script>
        $(document).ready(function() {
            ...
            const jdGenartorUniqueId = "bfc9e8cb-638b-447f-8a94-d6dfcd28aecb";

            // Toggle thumbs active state
           $('#thumbs-up, #thumbs-down').on('click', function () {
                feedbackType = this.id === 'thumbs-up' ? 'up' : 'down';
            ...
            $('input[name="accuracy"]').change(function() {
                $('.feedback-label').removeClass('selected');
            ...
            $('#feedbackModal').on('hidden.bs.modal', function() {
                $('.feedback-label').removeClass('selected');
                $('input[name="accuracy"]').prop('checked', false);
                $('.error-message').remove();
                $('#comments').removeClass('is-invalid');
            });
            ...
            $('#submit-feedback').click(function(e) {
                ...
                $('.error-message').remove(); // Remove any existing errors
                $('#comments').removeClass('is-invalid');

                const selectedAccuracy = $("input[name='accuracy']:checked");
                const additionalFeedbackInput = $('#comments');
                const additionalFeedback = additionalFeedbackInput.val().trim();
                ...
                // Validate additional comment
                if (!additionalFeedback) {
                    isValid = false;
                    additionalFeedbackInput.addClass('is-invalid');
                    additionalFeedbackInput.after(
                        '<p class="text-danger error-message">Additional Comments are required.</p>');
                }

                if (!isValid) return; // Stop if validation failed

                // Prepare payload
                const feedbackRating = selectedAccuracy.attr('id');
                const feedbackLabel = $(`label[for="${feedbackRating}"]`);
                const feedbackDescription = feedbackLabel.find('.heading-emotions-desc').text().trim();
                console.log("feedbackType:", feedbackType);

                const payload = {
                    job_generation_id: jdGenartorUniqueId,
                    thumbs_up: feedbackType === 'up' ? true : false,
                    thumbs_down: feedbackType === 'down' ? true : false,
            ...
            document.getElementById('submit-feedback').addEventListener('click', function(e) {
                ...
                const additionalCommentInput = document.getElementById('comments');
                const additionalComment = additionalCommentInput.value.trim();
                const feedbackType = document.querySelector('.thumbs-up.active') ? 1 : 0;

                // Validate input fields
                let isValid = true;
                document.querySelectorAll('.error-message').forEach(el => el
            .remove()); // Remove old error messages
                additionalCommentInput.classList.remove('is-invalid'); // remove red border if any
                ...
                if (!additionalComment) {
                    ...
                    const error = document.createElement('p');
                    error.classList.add('text-danger', 'error-message');
                    error.innerText = 'Additional Comments are required.';
                    additionalCommentInput.after(error);
                    additionalCommentInput.classList.add('is-invalid');
                }
                ...
                fetch('/admin/feedback/store', {
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            // $('#feedbackModal').modal('hide');
                            document.querySelector('#feedbackModal [data-bs-dismiss="modal"]').click();
    ...
    <script>
        ...
        document.querySelector('input[name="job_profile_name"]').textContent = jobRoleName || '';
    });
    </script>
