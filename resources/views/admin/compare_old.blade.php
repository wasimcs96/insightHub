...
@section('content')
    ...
    <div id="kt_app_toolbar" class="app-toolbar  py-3 py-lg-6 ">
        ...
        <div id="kt_app_toolbar_container" class="app-container  container-fluid d-flex flex-stack ">
            ...
            <span class="fs-7 fw-bold text-gray-700 flex-shrink-0 pe-4 d-none d-md-block">Employee A:</span>
            <!--end::Label-->
            @php
            ...
            <!--begin::Select-->
                <select class="form-select form-select-sm w-125px form-select-solid me-6" data-control="select2"
                    data-placeholder="Select Employee A" data-hide-search="true" name="employee_a" id="emp_a">
                    <option selected="selected" value=""
                        class="inline-block font-Inter font-normal text-sm text-slate-600">
                        Select Employee A</option>
                    @foreach ($employees as $employee)
                    <option value="{{ $employee->id }}"
                        class="inline-block font-Inter font-normal text-sm text-slate-600" {{
                        request('employee_a')==$employee->id ? 'selected' : '' }}>
                ...
                <!--end::Separartor-->
                <span class="fs-7 fw-bold text-gray-700 flex-shrink-0 pe-4 d-none d-md-block">Employee B:</span>
                <!--end::Label-->

                <!--begin::Select-->
                <select class="form-select form-select-sm w-125px form-select-solid me-6" data-control="select2"
                    data-placeholder="Select Employee A" data-hide-search="true" name="employee_b" id="emp_b">
                    <option selected="selected" value=""
                        class="inline-block font-Inter font-normal text-sm text-slate-600">
                        Select Employee B</option>
                    @foreach ($employees as $employee)
                    <option value="{{ $employee->id }}"
                        class="inline-block font-Inter font-normal text-sm text-slate-600" {{
                        request('employee_a')==$employee->id ? 'selected' : '' }}>
                ...
                <div class="bullet bg-secondary h-35px w-1px mx-6"></div>
                <!--end::Separartor-->
                <span class="fs-7 fw-bold text-gray-700 flex-shrink-0 pe-4 d-none d-md-block">Report:</span>
                <!--end::Label-->

                <!--begin::Select-->
                <select class="form-select form-select-sm w-125px form-select-solid me-6" data-control="select2"
                    data-placeholder="Select Report" data-hide-search="true" name="assessment" id="assessment">
                    <option selected="selected" value=""
                        class="py-1 inline-block font-Inter font-normal text-sm text-slate-600">
                        Select Report</option>
                    <option @if (request('assessment') == 'all_facets') selected="Selected" @endif value="all_facets"
                        class="py-1 inline-block font-Inter font-normal text-sm text-slate-600">
                        All Facets (OCEAN)</option>
                    <option @if (request('assessment') == 'ocean') selected="Selected" @endif value="ocean"
                        class="py-1 inline-block font-Inter font-normal text-sm text-slate-600">
                        Personality & Motivation (OCEAN)</option>

                    <option @if (request('assessment') == 'work_interest') selected="Selected" @endif
                        value="work_interest" class="py-1 inline-block font-Inter font-normal text-sm text-slate-600">
                        Work Interest (RIASEC)</option>
                    <option @if (request('assessment') == 'talent_pillar') selected="Selected" @endif
                        value="talent_pillar" class="py-1 inline-block font-Inter font-normal text-sm text-slate-600">
                        Talent Pillar
                    </option>
                    <option @if (request('assessment') == 'cognitive_ability') selected="Selected" @endif
                        value="cognitive_ability"
                        class="py-1 inline-block font-Inter font-normal text-sm text-slate-600">
                        Cognitive Ability
                    </option>
                    <option @if (request('assessment') == 'team_dynamics') selected="Selected" @endif
                        value="team_dynamics" class="py-1 inline-block font-Inter font-normal text-sm text-slate-600">
                        Team Dynamics
                    </option>
                ...
                <div class="d-flex align-items-center">
                    <button type="submit" class="btn btn-sm btn-icon btn-light-primary me-3" data-bs-toggle="tooltip"
                        ...
                        <iconify-icon icon="bx:git-compare" class="fa-2x"></iconify-icon>
                    </button>
            ...
            <div class="card">
                @if($isDepartment)
                    <div class="dashboard">
                        ...
                        <div class="sidebar">
                            <div style="margin-top: 24px;">Name of Department</div>
                            <div>No. of Persons in Department</div>
                            <div>Average Soft Skill Match Rate</div>
                            <div>Average Predictive Performance Score (%)</div>
                            <div>Average Cognitive Asessment Level</div>
                            <div>Average Growth Potential</div>
                            <div>Average Flight Risk</div>
                        </div>
                        <div class="cards">
                            <!-- Card 1 -->
                            @foreach ($data as $user_id => $single)
                                <div class="cardd">
                                    @php $title = \Illuminate\Support\Str::limit($single['name'], 10, '...'); @endphp
                                    <div class="match-rate cursor-pointer"  data-bs-toggle="tooltip"
                                    data-bs-placement="top" title="{{ $single['name'] }}"><span>{{ $title ?? '' }}</span></div>

                                    <div class="match-rate"><span>{{ $single['total_users'] ?? '0' }}</span></div>
                                    <div class="match-rate"><span>{{ $single['match_rate'] ?? '' }}%</span></div>

                                    <div class="predictive-score">
                                        <span>{{ $single['predictive_performance_result'] ?? '' }} %</span>
                                    </div>
                                    <div class="cognitive dim_title">Level: {{ $single['cognitive_ability_result'] ?? '0' }}</div>
                                    <div class="reliability dim_title">
                                       @if(isset($single['growth_potential_result']) && $single['growth_potential_result'] >  80)
                                       Super High
                                       @elseif(isset($single['growth_potential_result']) && $single['growth_potential_result'] >  60)
                                       Very High
                                       @elseif(isset($single['growth_potential_result']) && $single['growth_potential_result'] >  50)
                                        High
                                       @else
                                       Average
                                       @endif
                                    ...
                                    <div class="retention-risk dim_title" style="margin-top: 10px;margin-bottom: 0px;">
                                        {{ $single['flight_risk_result'] ?? '0' }} %</div>
                                </div>
                            @endforeach
                        </div>
                ...
                @else
                    <div class="dashboard">
                        ...
                        <div class="sidebar">
                            ...
                            <div >TP Match Rate</div>
                            
                            <div style="margin-top: 15px;">Position</div>
                        ...
                        <div class="cards">
                            ...
                            @foreach ($data as $user_id => $single)
                                <div class="cardd">
                                    ...
                                    data-bs-placement="top"
                                     >
                                     <span>{{ $single['talent_pillar_match_rate'] ?? '' }}%</span>
                                    </div>

                                    @php $position = \Illuminate\Support\Str::limit($single['position'], 10, '...'); @endphp
