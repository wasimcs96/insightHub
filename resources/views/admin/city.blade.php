        ...
        <form class="d-flex align-items-center overflow-auto" action="">
            ...
            {{-- <form action="d-flex align-items-center overflow-auto"> --}}
                ...
                <div class="d-flex align-items-center">
                    ...
                    <a href="/admin/bookmarks" class="btn btn-sm btn-icon btn-light me-3" data-bs-toggle="tooltip"
        ...
        <div class="card mb-5 mb-xl-8">
            ...
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bold fs-3 mb-1">{{ $city->name ?? 'Other' }}</span>
            ...
            <div class="card-body py-3">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table align-middle gs-0 gy-4">
                        ...
                        <!--begin::Table body-->
                        <tbody>
                            @foreach($users as $bookmark)
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">

                                        <div class="symbol symbol-50px me-5">
                                            <img src="{{asset($bookmark->profile_picture)}}" class="" alt="" />
                                        </div>

                                        <div class="d-flex justify-content-start flex-column">
                                            <a href="/admin/employee-details/{{$bookmark->id}}" class="text-gray-900 fw-bold text-hover-primary mb-1 fs-6">{{ $bookmark->first_name ?? ''}} {{ $bookmark->last_name ?? '' }}</a>
                                            <span class="text-muted fw-semibold text-muted d-block fs-7">{{ strtolower($bookmark->email) ?? ''}}</span>
                                        </div>
                                    </div>
                                </td>

                                <td>
                                    <span class="text-gray-900 text-center fw-bold text-hover-primary d-block mb-1 fs-6">{{ $bookmark->age ?? ''}}</span>

                                </td>

                                <td>

                                    <span class="text-gray-900 text-center fw-bold text-hover-primary d-block mb-1 fs-6">{{ $bookmark->mobile_number ?? ''}}</span>
                                </td>

                                <td>
                                    <span class="text-gray-900 text-center fw-bold text-hover-primary d-block mb-1 fs-6">{{ $bookmark->city ?? ''}}</span>
                                </td>

                                <td>
                                    <span class="text-gray-900 fw-bold text-center text-hover-primary d-block mb-1 fs-6">
                                         @if($bookmark->potential == 3)
                                ...
                                {{-- <td>
                                    <span class="text-gray-900 fw-bold text-hover-primary d-block mb-1 fs-6">{{ $bookmark->home_address ?? ''}}</span>

                                </td> --}}

                                <td>

                                    <span class="text-gray-900 fw-bold text-center text-hover-primary d-block mb-1 fs-6">{{ $bookmark->education_level_check->name ?? ''}}</span>
                                </td>

                                <td>
                                    <span class="text-gray-900 fw-bold text-center text-hover-primary d-block mb-1 fs-6">{{ $bookmark->higher_learning->name ?? ''}}</span>
                                </td>
                                <td>
                                    <span class="text-gray-900 fw-bold text-center text-hover-primary d-block mb-1 fs-6">{{ $bookmark->scope->name ?? ''}}</span>
                                </td>
                                <td>
                                    <span class="text-gray-900 fw-bold text-center text-hover-primary d-block mb-1 fs-6">{{ $bookmark->year_of_experience_in_it_sector ?? ''}}</span>
                                </td>
                                <td class="text-center">
                                    <a href="/admin/employee-details/{{$bookmark->id}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
...
<script>
    function exportToExcel() {
        ...
        XLSX.writeFile(wb, 'bookmarked_employees.xlsx');
    }
