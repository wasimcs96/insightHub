...
@section('content')
    ...
    <div id="kt_app_content" class="app-content  flex-column-fluid position-lg-relative">
        <div id="interviewInformation" class="app-container container-xxl w-100">
            @include('admin.talent-acquisition.candidate-screening.upcoming-interview.interview-information')
        </div>
    </div>
...
<script>
    ...
    {{-- <script>
        document.getElementById("conductInterviewBtn").addEventListener("click", function() {
            document.getElementById("interviewInformation").style.display = "none";
        ...
        document.getElementById("backToList").addEventListener("click", function() {
            document.getElementById("interviewInformation").style.display = "block";
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const selectedDiv = document.querySelector(".selected");
        const notSelectedDiv = document.querySelector(".not-selected");
        const hiddenInput = document.getElementById("jobOpeningApplicationId"); 
        const conductInterviewBtn = document.getElementById("conductInterviewBtn");
        ...
        leftTable.addEventListener("click", function (event) {
            ...
            conductInterviewBtn.addEventListener("click", function () {
                const applicationId = hiddenInput.value;
                ...
                window.location.href = `/admin/talent-acquisition/candidate-screening/conduct-interview?application_id=${applicationId}`;
            });
    });

    function fetchInterviewDetails(applicationId) {
        fetch(`/admin/talent-acquisition/candidate-screening/interview-info/${applicationId}`)
            .then(response => response.ok ? response.json() : response.text().then(text => { throw new Error(text); }))
            .then(data => {
                if (!data.success) throw new Error(data.message);
                updateInterviewDetails(data);
                hideOverlay();
            })
            // .catch(error => console.error('Error fetching interview details:', error));
            .catch(error => {
                console.error('Error fetching interview details:', error);
                hideOverlay(); // Hide overlay even if there is an error
            });
    }

    function updateInterviewDetails(data) {
        document.getElementById("candidateName").innerText = data.candidate_name;
        document.getElementById("interviewDateTime").innerText = `${data.interview_date} – ${data.interview_time}`;
        document.getElementById("appliedPosition").innerText = data.job_position;
        document.getElementById("interviewMode").innerText = data.interview_type;
        const joinInterviewContainer = document.querySelector(".join-interview");

        const interview_mode = data.interview_type; // "Online", "Phone", or "Physical"
        const interview_link = data.interview_link;

        const mode = interview_mode?.toLowerCase();

        joinInterviewContainer.innerHTML = "";

        if (mode === "online" && interview_link) {
            // Create icon element
            const icon = document.createElement("iconify-icon");
            icon.setAttribute("icon", "material-symbols:link");
            icon.setAttribute("width", "16");
            icon.setAttribute("height", "16");
            icon.style.marginRight = "5px"; // optional spacing

            // Create anchor element
            const link = document.createElement("a");
            link.setAttribute("href", interview_link);
            link.setAttribute("target", "_blank"); // open in new tab
            link.setAttribute("id", "interviewLink");
            link.textContent = "Join Interview";

            // Append both icon and link
            joinInterviewContainer.appendChild(icon);
            joinInterviewContainer.appendChild(link);
        } else {
            joinInterviewContainer.innerHTML = '<span>N/A</span>';
        }
        ...
        updateBadgeList("interviewers", data.interviewers);
        document.getElementById("jobRoleDepartment").innerText = data.job_role.department;
        document.getElementById("jobRoleName").innerText = data.job_role.name;
        document.getElementById("jobDescription").innerText = data.job_role.description;

        // Update Critical Work Functions
        updateAccordion("customAccordion", data.job_role.critical_work_functions);

        updateBadgeList("softSkills", data.soft_skills);
        updateBadgeList("technicalSkills", data.technical_skills);
        document.getElementById("companyOverview").innerText = data.company_overview;
        updateList("compensationBenefits", data.compensation_benefits);
    }

    function updateBadgeList(elementId, items) {
    ...
    function updateAccordion(elementId, functions) {
        const accordion = document.getElementById(elementId);
        accordion.innerHTML = functions.length > 0
            ? functions.map((func, index) => `
                <div>
                    <div class="accordion-design d-flex align-items-center" data-bs-toggle="collapse" data-bs-target="#function-${index}" aria-expanded="false">
                        <iconify-icon icon="oui:plus-in-circle-filled" width="23" height="23" class="icon plus-icon"></iconify-icon>
                        <iconify-icon icon="lsicon:minus-outline" width="23" height="23" class="icon minus-icon d-none"></iconify-icon>
                        ${func.description}
                    </div>
                    <div id="function-${index}" class="accordion-collapse collapse" data-bs-parent="#${elementId}">
                        <div class="accordion-body fs-6 ps-10 mt-2">
                                ${func.keys.length > 0 ? func.keys.map(key => `${key}`).join('<br>') : 'No keys available'}
                        </div>
                    </div>
                </div>
            `).join('')
            : `<p>No critical functions available</p>`;
    }

    function updateList(elementId, items) {
        document.getElementById(elementId).innerHTML = items.length > 0 
            ? items.map(item => `<li class="m-0 desc">${item}</li>`).join('')
            : `<p>No data available</p>`;
    }
