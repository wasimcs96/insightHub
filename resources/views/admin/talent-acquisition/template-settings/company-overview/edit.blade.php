...
@section('title', 'Edit Company/Department Overview')
    ...
    <div id="kt_app_content" class="app-content  flex-column-fluid position-lg-relative">
        <div id="kt_app_content_container" class="app-container  container-xxl  w-100">
            <div class="justify-content-between align-items-center success-message">
                <p class="fw-medium m-0"><b>Success!</b> Company Overview has been updated successfully.
                </p>
                <iconify-icon icon="iconamoon:close-bold" width="24" height="24" class="cursor-pointer"
                    id="closeIcon"></iconify-icon>
            </div>

            <h4 class="fw-medium">Edit Company/Department Overview</h4>
            <div class="template-tab">
                <form action="{{ route('admin.talent-acquisition.template-settings.company-overview.update', $overview->id) }}" class="w-100" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="mb-3">
                        <label>Company/Department Overview Name</label>
                        <input name="name" type="text" class="form-control" value="{{ old('name', $overview->name) }}" placeholder="Enter Company/Department Overview Name">
                    </div>
                    <div class="mb-8">
                        <label>Company/Department Overview</label>
                        <textarea id="overviewEditor" name="description" class="form-control">{{ old('description', $overview->description) }}</textarea>
                    </div>
