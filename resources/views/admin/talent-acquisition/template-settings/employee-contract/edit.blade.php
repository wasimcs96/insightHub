...
@section('title', 'Edit Employee Contract Template')
@section('content')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12">
                            <form action="{{ route('admin.talent-acquisition.template-settings.employee-contract.update',$contractTemplate->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row mb-3">
                                    <div class="col-lg-6">
                                        ...
                                        <input type="text" name="name"
                                            ...
                                            placeholder="Template Name" value="{{ old('name', isset($contractTemplate) ? $contractTemplate->name : '') }}"/>
                                        @error('name')
                                    ...
                                    <div class="col-lg-6">
                                        ...
                                        <textarea name="description"
                                            ...
                                            placeholder="Template Description">{{ old('description', isset($contractTemplate) ? $contractTemplate->description : '') }}</textarea>
                                        @error('description')
                                        <div class="invalid-feedback text-red-500">
                                            {{ $message }}
                                        </div>
                                ...
                                <div id="contract-details" class="mb-3">
                                    ...
                                    @isset($contractTemplate)
                                        @foreach ($contractTemplate->fields as $field)
                                            @if ($field->group_id == 'contract-details')
                                                <div class="row mb-3" id="field-{{ $loop->index }}">
                                                    <div class="col-lg-6">
                                                        <label class="fw-semibold fs-6 mb-2">{{ Str::title(str_replace('_', ' ', $field->field_name)) }}</label>
                                                        <input type="hidden" name="optional_fields[{{ $loop->index }}][key]" value="{{ $field->field_name }}">
                                                        <input type="hidden" name="optional_fields[{{ $loop->index }}][type]" value="{{ $field->field_type }}">
                                                        <input type="hidden" name="optional_fields[{{ $loop->index }}][group_id]" value="contract-details">
                                                        <div class="form-control form-control-solid mb-3 mb-lg-0">{{ ucfirst($field->field_type) }}</div>
                                                    </div>
                                                    <div class="col-lg-6">
                                        ...
                                        @endforeach
                                    @endisset
                                    <div class="row mb-3 add-optional-field-row">
                                        <div class="col-lg-2">
                                            ...
                                            @if ($field->group_id == 'compensation-benefits')
                                                <div class="row mb-3" id="field-{{ $loop->index }}">
                                                    <div class="col-lg-6">
                                                        ...
                                                        <input type="hidden" name="optional_fields[{{ $loop->index }}][key]" value="{{ $field->field_name }}">
                                                        <input type="hidden" name="optional_fields[{{ $loop->index }}][type]" value="{{ $field->field_type }}">
                                                        <input type="hidden" name="optional_fields[{{ $loop->index }}][group_id]" value="compensation-benefits">
                                                        <div class="form-control form-control-solid mb-3 mb-lg-0">{{ ucfirst($field->field_type) }}</div>
                                                    </div>
                                                    <div class="col-lg-6">
...
@section('scripts')
<script>
    let fieldIndex = {{ isset($contractTemplate) ? $contractTemplate->fields->count() : 1 }};
    ...
    function addSelectedField(select, sectionId) {
        ...
        if (selectedField) {
            ...
            const addOptionalFieldRow = sectionDiv.querySelector('.add-optional-field-row');
            sectionDiv.insertBefore(newFieldDiv, addOptionalFieldRow);
