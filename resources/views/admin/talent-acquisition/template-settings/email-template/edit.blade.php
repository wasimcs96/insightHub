...
<div id="kt_app_content" class="app-content flex-column-fluid position-lg-relative">
    <div id="kt_app_content_container" class="app-container container-xxl w-100">
        <div class="success-message justify-content-between align-items-center">
            <p class="fw-medium m-0"><b>Success!</b> Email Template updated successfully.</p>
            <iconify-icon icon="iconamoon:close-bold" width="24" height="24" class="cursor-pointer" id="closeIcon"></iconify-icon>
        </div>

        <div class="d-flex justify-content-between align-items-center">
            <h4 class="fw-medium">Update Email Template</h4>
        ...
        <div class="template-tab">
            <form action="{{ route('admin.talent-acquisition.template-settings.email-template.update', $template->id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label>Email Template Name</label>
                    <input type="text" class="form-control" value="{{ $template->title }}" name="title" placeholder="Enter Email Template Name">
                ...
                <div class="mb-1 d-flex justify-content-between align-items-center">
                    ...
                    <div class="dropdown">
                        <button class="btn-data-fields" type="button" data-bs-toggle="dropdown" aria-expanded="false" data-target="subject">
                ...
                <div class="mt-2 d-flex gap-1">
                    <input type="text" value="{{ $template->subject }}" class="form-control selectedFieldInput" name="subject" placeholder="Update on Your Application for">
                ...
                <div class="mb-3">
                    ...
                    <select class="custom-select" name="type" id="selectedType" style="width: 30%">
                        @foreach(config('constants.TYPE_FIELDS') as $key => $value)
                            <option value="{{ $key }}" {{ old('type', $template->type) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-1 d-flex justify-content-between align-items-center">
                    ...
                    <div class="dropdown">
                        <button class="btn-data-fields" type="button" data-bs-toggle="dropdown" aria-expanded="false" data-target="editor">
                            Insert Data Fields <iconify-icon icon="gg:chevron-down" width="16" height="16"></iconify-icon>
                        </button>
                        <ul class="dropdown-menu">
                ...
                <div class="mt-2 mb-8 gap-1">
                    <textarea id="example" name="description">{{ $template->description }}</textarea>
                </div>

                <div class="d-flex justify-content-end">
                    <button class="custom-button btn-apply">Update Template</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    // Insert data field only into correct target
    document.querySelectorAll(".data-field").forEach(item => {
        ...
        item.addEventListener("click", function (e) {
            ...
            if (target === 'subject') {
                const input = document.querySelector('.selectedFieldInput');
                if (input) {
