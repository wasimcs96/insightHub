...
<body>
    ...
    <div class="main">
        <form action="/admin/login/submit" method="POST" class="main__form">     
@csrf
            <div class="main__form__inputs">
              <div>  <img src="{{asset('images/CXS-Full-Tight.png')}}" style="width: 190px"></div>
                {{-- {{dd(session('error'))}} --}}
                @if (session('error'))
                <div class="alert alert-danger align-items-center justify-content-start " role="alert">
                    Invalid Credentials! May be email or password is invalid
                  </div>
                  @endif
                <div>
                    <input type="email" placeholder="EMAIL" name="email" class="email" value="{{ old('email') }}" required id="input1">
                    <span id="span1"></span>
                    @error('username')
