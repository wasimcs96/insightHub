    ...
    <div id="kt_app_toolbar_container" class="app-container  container-fluid d-flex flex-stack ">
        ...
        <div data-kt-swapper="true" data-kt-swapper-mode="{default: 'prepend', lg: 'prepend'}"
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <!--begin::Item-->
                <li class="breadcrumb-item capitalize text-muted">
                    Workplace Alignment Forecast </li>
        ...
        <form class="d-flex align-items-center overflow-auto" action="">
            ...
            <div class="d-flex align-items-center">
                ...
                <a href="/admin/organizational-fit-forecast" class="btn btn-sm btn-icon btn-light me-3" data-bs-toggle="tooltip" data-bs-placement="top" title="Reset">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
                            ...
                            <tbody class="text-gray-600 fw-semibold">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="text-gray-800 fw-bolder fs-6">{{ ucfirst($user->organizational_fit_forecast ?? '') . ' Risk' }}</div>
                                        </div>
                                    </td>
                                    <td>
...
@section('scripts')
    ...
    <script>
        var ctx = document.getElementById('riskLevelByDepartmentChart').getContext('2d');
        var riskLevelChart = new Chart(ctx, {
            type: 'bar',
            data: {
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['H', 'M', 'L'], // For Department (first graph), or ['Level 4', 'Level 3', 'Level 2', 'Level 1'] for Position (second graph)
                datasets: [{
                    label: {!! json_encode($resultsOfDepartment->pluck('department_name')) !!},
                    data: [8, 6, 7], // Replace with the actual data for 'ALL' or [9, 6, 6] for 'ALL' positions
                    backgroundColor: 'rgba(255, 99, 132, 0.2)',
                    stack: 'Stack 0'
                }, {
                    label: 'HR Dept', // Change to 'Level 4' for the second graph
                    data: [3, 4, 6], // Replace with the actual data for 'HR Dept' or [1, 1, 0] for 'Level 4'
                    backgroundColor: 'rgba(54, 162, 235, 0.2)',
                    stack: 'Stack 0'
                }, {
                    label: 'Accountancy Dept', // Change to 'Level 3' for the second graph
                    data: [5, 2, 1], // Replace with the actual data for 'Accountancy Dept' or [3, 2, 1] for 'Level 3'
                    backgroundColor: 'rgba(75, 192, 192, 0.2)',
                    stack: 'Stack 0'
                } // Add more datasets for each category
                ]
            },
            options: {
                scales: {
    ...
    <script>
        // Department data
        var departmentLabels = {!! json_encode($departmentData->pluck('department_name')) !!};
        var departmentHighData = {!! json_encode($departmentData->pluck('high')) !!};
        var departmentModerateData = {!! json_encode($departmentData->pluck('moderate')) !!};
        var departmentLowData = {!! json_encode($departmentData->pluck('low')) !!};
    
        // Position data
        var positionLabels = {!! json_encode($positionData->pluck('position_name')) !!};
        var positionHighData = {!! json_encode($positionData->pluck('high')) !!};
        var positionModerateData = {!! json_encode($positionData->pluck('moderate')) !!};
        var positionLowData = {!! json_encode($positionData->pluck('low')) !!};
    
        // Create the Department Chart
        function filterTableDepartment(department) {
            var url = new URL(window.location.href);
            url.searchParams.set('department', department);
            url.hash = 'table-section';
        ...
        var departmentChart = new Chart(ctxDepartment, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'High Risk',
                    data: departmentHighData,
                    backgroundColor: 'rgba(255, 99, 132, 0.2)',
                    stack: 'Stack 0'
                }, {
                    label: 'Moderate Risk',
                    data: departmentModerateData,
                    backgroundColor: 'rgba(54, 162, 235, 0.2)',
                    stack: 'Stack 0'
                }, {
                    label: 'Low Risk',
                    data: departmentLowData,
                    backgroundColor: 'rgba(75, 192, 192, 0.2)',
                    stack: 'Stack 0'
                }]
            },
            options: {
                ...
                onClick: function (e) {
                    var elements = this.getElementsAtEventForMode(e, 'nearest', { intersect: true }, false);
                    if (elements.length > 0) {
                        var element = elements[0];
                        var department = this.data.labels[element.index];
        ...
        function filterTableRiskLevel(riskLevel) {
            
            var formattedRiskLevel = riskLevel.toLowerCase().split(' ')[0];

            var url = new URL(window.location.href);
            url.searchParams.set('risk_level', formattedRiskLevel); // Add the formatted risk level to the URL
            url.hash = 'table-section';
            window.location.href = url.toString();
        }
        ...
        var positionChart = new Chart(ctxPosition, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'High Risk',
                    data: positionHighData,
                    backgroundColor: 'rgba(255, 99, 132, 0.2)',
                    stack: 'Stack 0'
                }, {
                    label: 'Moderate Risk',
                    data: positionModerateData,
                    backgroundColor: 'rgba(54, 162, 235, 0.2)',
                    stack: 'Stack 0'
                }, {
                    label: 'Low Risk',
                    data: positionLowData,
                    backgroundColor: 'rgba(75, 192, 192, 0.2)',
                    stack: 'Stack 0'
                }]
            },
            options: {
                ...
                onClick: function (e) {
                    ...
                    if (elements.length > 0) {
                        ...
                        var riskLevel = this.data.datasets[datasetIndex].label;

                        // Pass only the risk level
                        filterTableRiskLevel(riskLevel);
                    }
                }
            }
        });
