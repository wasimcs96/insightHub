...
<div id="kt_app_content" class="app-content flex-column-fluid">
    <div id="kt_app_content_container" class="app-container container-xxl">
        <div class="d-flex flex-column flex-lg-row mb-17">
            ...
            <form method="POST" action="{{ route('performance.management.updateSkills', ['reviewId' => $review->id]) }}">
        @csrf
        @method('PUT')    
        <div class="skill-row justify-content-between mb-8 mt-8">
    <div class="rating justify-content-around col-lg-12">
        <div class="col-lg-2">
            <label for="kpi_level" class="fw-bold text-gray-800 fs-5">{{ $review->name }}</label>
        </div>
        @for ($i = 1; $i <= 4; $i++)
        <div class="d-flex flex-column-reverse form-check form-check-custom form-check-warning form-check-solid">
            <input class="mt-10 form-check-input checkbox_kpi" type="radio" id="kpi_level_rate{{ $i }}" name="kpi_level" value="{{ $i }}" {{ $review->level == $i ? 'checked' : '' }}>
            <label for="kpi_level_rate{{ $i }}" class="fs-5 fw-bold">{{ $i }}</label>
        </div>
        @endfor
        <div class="col-lg-3">
            <p class="fs-5 fw-semibold">Remark</p>
            <textarea type="text" name="kpi_achievement" placeholder="Remarks..." class="remarks form-control form-control-solid" data-kt-autosize="true">{{ $review->remark }}</textarea>
        </div>
        <div class="col-lg-3">
            <p class="fs-5 fw-semibold">Manager Evaluation</p>
            <textarea type="text" disabled class="remarks form-control form-control-solid" data-kt-autosize="true">{{ $review->manager_evaluation }}</textarea>
        </div>
    </div>
</div>
    ...
    <div class="d-flex justify-content-between">
        <button onclick="document.getElementById('step3form').submit();" class="sticky-button btn btn-primary">Update</button>
    </div>
</div>
    </form>
@endsection
