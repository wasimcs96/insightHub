    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 mb-5 mb-lg-0">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    <li class="breadcrumb-item text-muted">
                        <a href="{{ url()->previous() }}" class="text-muted text-hover-primary">Job Advertisement</a>
                    </li>
            ...
            <form class="d-flex align-items-center overflow-auto" action="" method="GET">
                ...
                <select class="form-select form-select-sm form-select-solid me-6" data-control="select2" name="department_id" id="department_id" required>
                    @foreach($departments as $department)
                        <option value="{{ $department->id }}" @if(request('department_id') == $department->id) selected @endif>{{ $department->name }}</option>
                    @endforeach
                </select>
            
                <!-- 2. Job Advertisement filter -->
                <select class="form-select form-select-sm form-select-solid me-6" data-control="select2" name="job_opening_id" id="job_opening_id" required>
                    @foreach($job_openings as $job)
                        <option value="{{ $job->id }}" @if(request('job_opening_id') == $job->id) selected @endif>{{ $job->job_title }}</option>
                    @endforeach
                </select>
            
                <!-- 3. Pool Type filter -->
                <select class="form-select form-select-sm form-select-solid me-6" data-control="select2" name="pool_type" id="pool_type" required>
                    <option value="candidate_vs_candidate" @if(request('pool_type') == 'candidate_vs_candidate') selected @endif>Candidate vs Candidate</option>
                    <option value="candidate_vs_employee" @if(request('pool_type') == 'candidate_vs_employee') selected @endif>Candidate vs Employee</option>
                </select>
            
                <!-- 4. Application Status filter (only for Candidate vs Candidate) -->
                <select class="form-select form-select-sm form-select-solid me-6" data-control="select2" name="application_status" id="application_status" required>
                ...
                <!-- 5. Candidates filter -->
                <select class="form-select form-select-sm form-select-solid me-6" data-control="select2" name="candidate_ids[]" id="candidate_ids" multiple required>
                    <!-- Dynamic options -->
                </select>
            
                <!-- 6. Employees filter (only for Candidate vs Employee) -->
                <select class="form-select form-select-sm form-select-solid me-6" data-control="select2" name="employee_ids[]" id="employee_ids" multiple style="display: none;">
                    <!-- Dynamic options -->
                </select>
                ...
                <div class="card-header border-0">
                    ...
                    <div class="card-toolbar">
                        <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                            <a href="{{ route('admin.job-openings.index', ['department_id' => session('job_opening_department_id')]) }}" class="btn btn-primary d-flex align-items-center">
                            ...
                            @php
                                $fields = [
                                    ...
                                    'Application Status' => [
                                        ...
                                        'value' => function($single) {
                                            return config('helpers.application_status')[$single['job_opening_application_status']] ?? 'N/A';
                                        },
                                        'description' => function($single) {
                                            $value = config('helpers.application_status')[$single['job_opening_application_status']] ?? 'N/A';
                                            return $value;
                                        }
                                    ],
                                ];
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function handleMaxSelection() {
            ...
            if (totalSelected > maxSelections) {
                ...
                this.value = '';  // Reset the select to prevent the overflow
            }
        }
