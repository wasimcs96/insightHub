    ...
    <div id="kt_app_toolbar_container" class="app-container  container-fluid d-flex flex-stack ">
        ...
        <div data-kt-swapper="true" data-kt-swapper-mode="{default: 'prepend', lg: 'prepend'}"
            ...
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <!--begin::Item-->
                <li class="breadcrumb-item text-muted">
                    <a href="/admin/department/users" class="capitalize text-muted text-hover-primary">
                        Department
                    </a>
                </li>
                <!--begin::Item-->
                ...
                <!--begin::Item-->
                <li class="breadcrumb-item text-muted">
                    {{ !empty($user) ? 'Edit Department' : 'Create Department' }} </li>
                <!--end::Item-->
    ...
    <div id="kt_app_content_container" class="app-container  container-xxl ">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form class="form" action="/admin/department/user/{{ !empty($user) ? $user->id . '/update' : 'store' }}"
                    ...
                    <div class=" ">
                        ...
                        <div class="fv-row mb-7 row">
                            ...
                            <div class="col-md-6 fv-row">
                                ...
                                <select class="form-select form-select-solid" id="company" data-control="select2"
                                    data-hide-search="false" data-placeholder="Select a Company" name="company_id">
                                    <option value="">Select Company...</option>

                                    @foreach($company as $com)
                                    <option value="{{ $com->id }}" @if(!empty($user)&&$user->company_id == $com->id) selected @endif>{{ $com->name ?? '' }}</option>
                                    @endforeach
                                </select>
                            </div>
                        <div class="col-md-6 fv-row">
                            ...
                            <select class="form-select form-select-solid" id="sector" data-control="select2"
                                data-hide-search="false" data-placeholder="Select a Sector"@if(!empty($user)) disabled @endif  name="sector_id">
                                <option value="">Select a Sector...</option>

                                @foreach($department as $dep)
                                <option value="{{ $dep->id }}" @if(!empty($user)&&$user->sierra_id == $dep->id) selected @endif>{{ $dep->name ?? '' }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="fv-row mb-7 row">
                            ...
                            <div class="col-lg-6">
                                ...
                                <input type="text" name="last_name"
                                    ...
                                    placeholder="Last Name" value="{{ !empty($user) ? $user->user->last_name : old('last_name') }}" />
                        ...
                        <div class="fv-row mb-7 col-lg-4">
                            ...
                            <input type="email" name="email"
                                ...
                                placeholder="example@domain.com" value="{{ !empty($user) ? $user->user->email : old('email') }}" />
                            @error('email')
                            <div class="invalid-feedback text-red-500">
                                {{ $message }}
                            </div>
                        ...
                        <div class="col-lg-4">
                            ...
                            <input type="text" name="head_of_department"
                                class="form-control form-control-solid mb-3 mb-lg-0 @error('head_of_department') is-invalid @enderror"
                                placeholder="Head Of Department" value="{{ !empty($user) ? $user->head_of_department : old('head_of_department') }}" />
                        ...
                        <div class="col-lg-4">
                            ...
                            <!--begin::Input-->
                            <input type="text" name="number_of_pax"
                                class="form-control form-control-solid mb-3 mb-lg-0 @error('number_of_pax') is-invalid @enderror"
                                placeholder="Number of Pax" value="{{ !empty($user) ? $user->number_of_pax : old('number_of_pax') }}" />
...
<script>
    $(document).ready(function() {
        $('#company').change(function() {
            ...
            $.ajax({
                url: '/admin/sectorget/' + companyId,
                type: 'GET',
                success: function(response) {
                    $('#sector').empty(); // Clear the previous sectors
                    $('#sector').append('<option value="">Select a Sector</option>'); // Optional: Add a placeholder

                    $.each(response.new_sectors, function(index, sector) {
                        // Assume each sector is an object with id and name
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#sector').append('<option value="">Error loading sectors</option>');
                }
            });
        });
    });
